...
import {
  ...
  type ContentRef,
  type SpellCatalogResponse,
} from "../index.js";
import { ContractValidationError } from "../validation.js";

const digest = "a".repeat(64);
const fireBoltRef: ContentRef = {
...
const spellCatalog: SpellCatalogResponse = {
...
test("content catalog accepts the authored tint field and spell catalog exposes the same exact ref", async () => {
  const client = new DndEngineClient("/api", {
    fetchImplementation: async (input) => (
      String(input).endsWith("/catalog/spells")
  ...
  assert.deepEqual(
    decodedContent.safe_presentations[0]?.ref,
    { presentation_contract_hash: digest },
  );
...
test("content catalog requires exact compound equipment render-layer rows", async () => {
  const equipmentCatalog: ContentCatalogResponse = {
    ...catalog,
  ...
  const decoded = await client.getContentCatalog();
  assert.deepEqual(
    decoded.safe_presentations[0]?.presentation.equipment_sprites,
    [
      {
        equipment_slot: "body_armor",
        render_layer: "chest",
  ...
  const invalidCatalog = structuredClone(catalog) as unknown as {
    safe_presentations: Array<{ presentation: Record<string, unknown> }>;
  };
  ...
  await assert.rejects(
    invalidClient.getContentCatalog(),
    ContractValidationError,
  );

  const retiredSlotCatalog = structuredClone(catalog) as unknown as {
    safe_presentations: Array<{
      presentation: {
        equipment_sprites: Array<Record<string, unknown>>;
      };
    }>;
  };
  ...
  await assert.rejects(
    retiredClient.getContentCatalog(),
    ContractValidationError,
  );
});
...
test("content and spell decoders reject retired compatibility shapes", async () => {
  ...
  await assert.rejects(
    catalogClient.getContentCatalog(),
    ContractValidationError,
  );
