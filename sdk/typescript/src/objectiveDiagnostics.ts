...
import {
  ...
  assertObjectiveCombatLogFrame,
  assertObjectiveGameEventFrame,
  assertObjectiveProtocolIdentity,
  type ObjectiveDiagnosticsSseEnvelope,
...
export class ObjectiveDiagnosticsClient {
  ...
  constructor(
    baseUrl: string,
    options: ObjectiveDiagnosticsClientOptions = {},
  ) {
  ...
  async bootstrap(signal?: AbortSignal): Promise<ObjectiveDiagnosticsBootstrap> {
    const bootstrap = decodeModel(
      "ObjectiveDiagnosticsBootstrap",
      await this.getJson(
      OBJECTIVE_DIAGNOSTICS_ROUTES.bootstrap,
      signal,
      ),
    );
  ...
  async events(
    query: ObjectiveDiagnosticsWindowQuery = {},
    signal?: AbortSignal,
  ): Promise<GameEventFramesResponse> {
    const response = decodeModel(
      "GameEventFramesResponse",
      await this.getJson(
      withWindowQuery(OBJECTIVE_DIAGNOSTICS_ROUTES.events, query),
      signal,
      ),
    );
  ...
  async combatLog(
    query: ObjectiveDiagnosticsWindowQuery = {},
    signal?: AbortSignal,
  ): Promise<ObjectiveCombatLogFramesResponse> {
    const response = decodeModel(
      "ObjectiveCombatLogFramesResponse",
      await this.getJson(
      withWindowQuery(OBJECTIVE_DIAGNOSTICS_ROUTES.combatLog, query),
      signal,
      ),
    );
  ...
  async subjectiveParity(
    sessionId: string,
    signal?: AbortSignal,
  ): Promise<SubjectiveRenderParityDiagnosticsResponse> {
    ...
    const response = decodeModel(
      "SubjectiveRenderParityDiagnosticsResponse",
      await this.getJson(
        `${OBJECTIVE_DIAGNOSTICS_ROUTES.subjectiveParity}?${parameters.toString()}`,
        signal,
      ),
    );
  ...
  async *subscribe(
    position: ObjectiveDiagnosticsStreamPosition,
    signal?: AbortSignal,
  ): AsyncGenerator<ObjectiveDiagnosticsSseEnvelope> {
    const parameters = new URLSearchParams({
    ...
    const response = await this.fetchImplementation(
      `${this.baseUrl}${OBJECTIVE_DIAGNOSTICS_ROUTES.subscribe}?${parameters.toString()}`,
      request,
    );
    if (!response.ok) {
  ...
  private async getJson(
    ...
    const request: RequestInit = signal === undefined
  ...
  private async httpError(response: Response): Promise<ObjectiveDiagnosticsHttpError> {
...
export function assertObjectiveDiagnosticsBootstrap(
  bootstrap: ObjectiveDiagnosticsBootstrap,
): void {
  ...
  if (encounter === null) {
    throw new ContractValidationError(
      "$objective.bootstrap.world.state.encounter",
      "objective diagnostics require an encounter",
    );
  }
  if (encounter.uuid !== bootstrap.source_stream_id) {
    throw new ContractValidationError(
      "$objective.bootstrap.world.state.encounter.uuid",
      "encounter UUID must equal the objective source stream",
    );
  }
}

export function assertSubjectiveRenderParityDiagnostics(
  response: SubjectiveRenderParityDiagnosticsResponse,
): void {
  if (response.projection !== "subjective_parity") {
    throw new ContractValidationError(
      "$subjectiveParity.projection",
      "expected subjective_parity",
    );
  }
  ...
  requireNonEmpty(
    response.perspective_epoch_id,
    "$subjectiveParity.perspective_epoch_id",
  );
  ...
  const comparedPaths = requireCursor(
    response.compared_path_count,
    "$subjectiveParity.compared_path_count",
  );
  if (comparedPaths < 1) {
    throw new ContractValidationError(
      "$subjectiveParity.compared_path_count",
      "comparison must exercise at least one manifest path",
    );
  }
  const structuralEdges = requireCursor(
    response.structural_edge_count,
    "$subjectiveParity.structural_edge_count",
  );
  const doorEdges = requireCursor(
    response.door_edge_count,
    "$subjectiveParity.door_edge_count",
  );
  requireCursor(response.visible_tile_count, "$subjectiveParity.visible_tile_count");
  if (doorEdges > structuralEdges) {
    throw new ContractValidationError(
      "$subjectiveParity.door_edge_count",
      "door edge count cannot exceed structural edge count",
    );
  }
  if (response.non_empty_structural_edges !== (structuralEdges > 0)) {
    throw new ContractValidationError(
      "$subjectiveParity.non_empty_structural_edges",
      "flag must match structural edge count",
    );
  }
  ...
  if (response.matches !== expectedMatches) {
    throw new ContractValidationError(
      "$subjectiveParity.matches",
      "status must match digests and mismatch list",
    );
  }
}

export function assertObjectiveGameEventWindow(response: GameEventFramesResponse): void {
  ...
  response.frames.forEach((frame, offset) => {
    ...
    if (frame.event_cursor !== expectedCursor) {
      throw new ContractValidationError(
        `$objective.events.frames[${offset}].event_cursor`,
        `expected contiguous cursor ${expectedCursor}`,
      );
    }
    if (frame.combat_log_cursor < combatLogBarrier) {
      throw new ContractValidationError(
        `$objective.events.frames[${offset}].combat_log_cursor`,
        "combat-log barriers must be nondecreasing",
      );
    }
...
export function assertObjectiveCombatLogWindow(
  response: ObjectiveCombatLogFramesResponse,
): void {
  ...
  if (response.perspective_epoch_id !== "objective") {
    throw new ContractValidationError(
      "$objective.combat_log.perspective_epoch_id",
      "expected the canonical objective epoch",
    );
  }
  ...
  response.frames.forEach((frame, offset) => {
    ...
    if (frame.combat_log_cursor !== expectedCursor) {
      throw new ContractValidationError(
        `$objective.combat_log.frames[${offset}].combat_log_cursor`,
        `expected contiguous cursor ${expectedCursor}`,
      );
    }
    if (frame.event_cursor < eventBarrier) {
      throw new ContractValidationError(
        `$objective.combat_log.frames[${offset}].event_cursor`,
        "event barriers must be nondecreasing",
      );
    }
...
function assertWindowBounds(response: WindowBounds, path: string): void {
  ...
  if (!(retained <= from && from <= through && through <= total)) {
    throw new ContractValidationError(
      path,
      "expected retained_from_cursor <= from_cursor <= through_cursor <= total",
    );
  }
  if (response.frames.length !== through - from) {
    throw new ContractValidationError(
      `${path}.frames`,
      `expected exactly ${through - from} contiguous frames`,
    );
  }
}

function assertWindowIdentity(
  response: WindowIdentity,
  frame: WindowIdentity,
  path: string,
): void {
...
function assertExpectedIdentity(
  ...
  if (
    expected.expectedSourceStreamId !== undefined
    && response.source_stream_id !== expected.expectedSourceStreamId
  ) {
    throw new ContractValidationError(
      `${path}.source_stream_id`,
      "response does not match expected source stream",
    );
  }
  if (
    expected.expectedGenerationId !== undefined
    && response.generation_id !== expected.expectedGenerationId
  ) {
    throw new ContractValidationError(
      `${path}.generation_id`,
      "response does not match expected generation",
    );
  }
}

function withWindowQuery(path: string, query: ObjectiveDiagnosticsWindowQuery): string {
  const parameters = new URLSearchParams();
...
function appendExpectedIdentity(
  ...
  if (identity.expectedSourceStreamId !== undefined) {
    parameters.set(
      ...
      requireNonEmpty(identity.expectedSourceStreamId, "$query.expectedSourceStreamId"),
    );
  }
  if (identity.expectedGenerationId !== undefined) {
    parameters.set(
      "expected_generation_id",
      requireNonEmpty(identity.expectedGenerationId, "$query.expectedGenerationId"),
    );
  }
}
...
function appendCursor(
  ...
  if (value !== undefined) parameters.set(name, String(requireCursor(value, path)));
}
