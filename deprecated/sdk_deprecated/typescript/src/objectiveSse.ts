...
export class ObjectiveDiagnosticsSseDecoder {
  private readonly syntaxDecoder = new SseDecoder();
...
export class ObjectiveDiagnosticsFollower {
  private sourceStreamId: string | null;
  private generationId: string | null;
  private eventCursor: number;
  private combatLogCursor: number;
  private eventCombatLogCursor = -1;
  private logEventCursor = -1;
  ...
  constructor(start: ObjectiveDiagnosticsFollowerStart = {}) {
    ...
    this.combatLogCursor = requireCursor(
      start.combatLogCursor ?? 0,
      "$objective.combat_log_cursor",
    );
  ...
  ingest(envelope: ObjectiveDiagnosticsSseEnvelope): ObjectiveDiagnosticsCursor {
    switch (envelope.event) {
      ...
      case "game_event":
        ...
        this.assertIdentity(
          envelope.data.source_stream_id,
          envelope.data.generation_id,
          "$objective.game_event",
        );
        if (envelope.data.event_cursor !== this.eventCursor + 1) {
          throw new ContractValidationError(
            "$objective.game_event.event_cursor",
            `expected contiguous cursor ${this.eventCursor + 1}`,
          );
        }
        if (envelope.data.event_index + 1 !== envelope.data.event_cursor) {
          throw new ContractValidationError(
            "$objective.game_event.event_index",
            "event_index must be exactly event_cursor - 1",
          );
        }
        if (envelope.data.combat_log_cursor < this.eventCombatLogCursor) {
          throw new ContractValidationError(
            "$objective.game_event.combat_log_cursor",
            "event combat-log barriers must be nondecreasing",
          );
        }
      ...
      case "combat_log":
        ...
        this.assertIdentity(
          envelope.data.source_stream_id,
          envelope.data.generation_id,
          "$objective.combat_log",
        );
        if (envelope.data.combat_log_cursor !== this.combatLogCursor + 1) {
          throw new ContractValidationError(
            "$objective.combat_log.combat_log_cursor",
            `expected contiguous cursor ${this.combatLogCursor + 1}`,
          );
        }
        if (envelope.data.event_cursor < this.logEventCursor) {
          throw new ContractValidationError(
            "$objective.combat_log.event_cursor",
            "combat-log event barriers must be nondecreasing",
          );
        }
        this.commitIdentity(envelope.data.source_stream_id, envelope.data.generation_id);
  ...
  private ingestSync(sync: ObjectiveDiagnosticsSync): void {
    ...
    if (sync.event_cursor < this.eventCursor) {
      throw new ContractValidationError(
        "$objective.sync.event_cursor",
        "source event cursor is behind the requested cursor",
      );
    }
    if (sync.combat_log_cursor < this.combatLogCursor) {
      throw new ContractValidationError(
        "$objective.sync.combat_log_cursor",
        "source combat-log cursor is behind the requested cursor",
      );
    }
    this.commitIdentity(sync.source_stream_id, sync.generation_id);
  }

  private assertIdentity(source: string, generation: string, path: string): void {
    ...
    if (this.generationId !== null && generation !== this.generationId) {
      throw new ContractValidationError(
        `${path}.generation_id`,
        "objective event generation changed",
      );
    }
  }

  private commitIdentity(source: string, generation: string): void {
    this.sourceStreamId = source;
    this.generationId = generation;
  }
}

export function decodeObjectiveDiagnosticsEnvelope(
  message: SseMessage,
): ObjectiveDiagnosticsSseEnvelope {
  switch (message.event) {
    ...
    default:
      throw new ContractValidationError(
        "$objective_sse.event",
        `unsupported objective diagnostics event ${message.event}`,
      );
  }
}

export function assertObjectiveGameEventFrame(
  frame: GameEventFrame,
  path = "$objective.game_event",
): void {
...
export function assertObjectiveCombatLogFrame(
  frame: ObjectiveCombatLogFrame,
  path = "$objective.combat_log",
): void {
  requireNonEmpty(frame.source_stream_id, `${path}.source_stream_id`);
  requireNonEmpty(frame.generation_id, `${path}.generation_id`);
