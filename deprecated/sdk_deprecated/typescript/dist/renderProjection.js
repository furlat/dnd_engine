import { assertGridConnectorStructure, assertSubjectiveWorld, } from "./reducer.js";
...
const FLOOR_OBJECT_KINDS = new Set([
    "item",
    "interactable",
    "container",
    "hazard",
...
const VISUAL_SLOT_SET = new Set(VISUAL_SLOTS);
...
export function projectSubjectiveRenderWorld(world, perspective) {
    ...
    const observerUuids = sortedUnique(perspective.observer_entity_uuids, "$render.subjective.observer_entity_uuids");
    return {
        projection: "subjective",
        perspective: {
            kind: perspective.kind,
            controlled_entity_uuids: sortedUnique(perspective.controlled_entity_uuids, "$render.subjective.controlled_entity_uuids"),
        ...
        knowledge: mergeObserverVisibility(world.visibility, observerUuids, perspective.active_observer_uuid),
        equipment: {
            detail_scope: "controlled",
            details_by_entity: projectEquipmentDetails(world.equipment_by_entity),
            visual_loadout_by_entity: projectVisualLoadouts(world.visual_loadout_by_entity),
        },
    };
}
/** Normalize an objective diagnostic world for debug UI and parity oracles. */
export function projectObjectiveRenderWorld(world, options = {}) {
    ...
    const observerUuids = sortedUnique(requestedObservers, "$render.objective.observer_entity_uuids");
    ...
    const visualLoadouts = {};
    for (const entity of [...world.state.entities].sort(compareUuid)) {
        visualLoadouts[entity.uuid] = deriveVisualLoadout(entity.uuid, world.equipment_by_entity[entity.uuid]);
    }
    return {
        ...
        knowledge: mergeObserverVisibility(world.visibility, observerUuids, activeObserverUuid),
        equipment: {
 ...
 */
export function mergeObserverVisibility(visibility, observerEntityUuids, activeObserverUuid = null) {
    const observerUuids = sortedUnique(observerEntityUuids, "$render.visibility.observer_entity_uuids");
    if (activeObserverUuid !== null
        && !observerUuids.includes(activeObserverUuid)) {
        throw new ContractValidationError("$render.visibility.active_observer_uuid", "active observer must belong to the selected observer union");
    }
    const observers = [];
    ...
    for (const observerUuid of observerUuids) {
        ...
        if (row === undefined) {
            throw new ContractValidationError(`$render.visibility.${observerUuid}`, "selected observer has no visibility row");
        }
        ...
        for (const cell of row.visible_cells) {
            ...
            if (light === undefined) {
                throw new ContractValidationError(`$render.visibility.${observerUuid}.effective_light_levels.${key}`, "visible cell has no effective-light value");
            }
    ...
    for (const position of sortedVisibleCells) {
        ...
        if (value === undefined) {
            throw new ContractValidationError(`$render.visibility.effective_light_levels.${key}`, "unioned visible cell has no effective-light value");
        }
 ...
 */
export function projectStructuralEdges(tiles) {
    const candidates = new Map();
    for (const tile of [...tiles].sort(compareTile)) {
        for (const direction of DIRECTIONS) {
            const blockedChannels = CHANNELS.filter((channel) => (tileBlocks(tile, channel, direction)));
            const appearance = tile.directional_structural_edges[direction];
            if (appearance !== null) {
                assertStructuralEdgeAppearance(appearance, `$render.tile[${tile.x},${tile.y}].directional_structural_edges.${direction}`);
            }
            if (blockedChannels.length === 0 && appearance === null)
                continue;
            const owned = canonicalEdgeOwner([tile.x, tile.y], direction);
            const key = canonicalRenderStructuralEdgeKey(owned.position, owned.direction);
...
export function canonicalRenderStructuralEdgeKey(position, direction) {
...
export function projectTraversalConnectors(connectors) {
    ...
    })).sort((left, right) => (compareString(left.authored_id, right.authored_id)
        || compareString(left.uuid, right.uuid)));
}
/** Derive the safe appearance layer used by the objective debug projection. */
export function deriveVisualLoadout(entityUuid, equipment) {
    const layers = [];
    for (const slot of equipment?.slots ?? []) {
        if (slot.item === null || !VISUAL_SLOT_SET.has(slot.slot))
            continue;
    ...
    return {
        ...
        active_weapon_set: (equipment === undefined ? "none" : equipment.active_weapon_set),
        layers,
    };
}
...
function projectEncounter(encounter) {
    if (encounter === null)
        return null;
    return {
        uuid: encounter.uuid,
...
function projectSubjectiveFloorObject(object) {
    return {
        uuid: object.uuid,
        name: object.name,
...
function projectObjectiveFloorObject(object) {
    const state = object.state;
    const directions = readEnumArray(state.blocked_directions, DIRECTION_SET, "$render.objective.floor_object.blocked_directions");
    const channels = readEnumArray(state.blocked_channels, CHANNEL_SET, "$render.objective.floor_object.blocked_channels");
    return {
        uuid: object.uuid,
        name: object.name,
        position: copyPosition(object.position),
        ...
        safe_presentation_ref: readSafePresentationRef(state.safe_presentation_ref),
        ...
        blocks_vision: (readBoolean(state.blocks_vision)
...
function objectiveObjectKind(state, directions, channels) {
    const explicit = readString(state.object_kind);
    if (explicit !== null && FLOOR_OBJECT_KINDS.has(explicit)) {
        return explicit;
    }
    if (readBoolean(state.is_open) !== null)
        return "door";
    if (readBoolean(state.is_lit) !== null
    ...
    if (readBoolean(state.is_container) === true)
        return "container";
    if (readStringArray(state.tags).includes("hazard"))
        return "hazard";
    const isUsable = readBoolean(state.is_usable) === true;
    const isPickable = readBoolean(state.is_pickable) === true;
    if (isUsable && !isPickable)
        return "interactable";
    if (isPickable)
        return "item";
    return "generic";
}
function projectEquipmentDetails(details) {
    const result = {};
    for (const entityUuid of Object.keys(details).sort(compareString)) {
        ...
        if (equipment === undefined)
            continue;
        result[entityUuid] = {
            slots: [...equipment.slots].sort((left, right) => (compareVisualSlot(left.slot, right.slot))),
...
function projectVisualLoadouts(loadouts) {
    const result = {};
...
function assertObjectiveWorld(world) {
    const grid = world.state.grid;
    if (grid.min_x > grid.max_x || grid.min_y > grid.max_y) {
        throw new ContractValidationError("$render.objective.state.grid", "grid bounds are reversed");
    }
    const tileKeys = new Set();
    for (const tile of grid.tiles) {
        const key = `${tile.x},${tile.y}`;
        if (tileKeys.has(key)) {
            throw new ContractValidationError("$render.objective.state.grid.tiles", "grid tile positions must be unique");
        }
        tileKeys.add(key);
        if (tile.x < grid.min_x
            || tile.x > grid.max_x
            || tile.y < grid.min_y
            || tile.y > grid.max_y) {
            throw new ContractValidationError("$render.objective.state.grid.tiles", "grid tile lies outside declared bounds");
        }
    }
    const entityUuids = new Set();
    for (const entity of world.state.entities) {
        if (entityUuids.has(entity.uuid)) {
            throw new ContractValidationError("$render.objective.state.entities", "entity UUIDs must be unique");
        }
    ...
    for (const object of world.state.floor_objects) {
        if (objectUuids.has(object.uuid)) {
            throw new ContractValidationError("$render.objective.state.floor_objects", "floor-object UUIDs must be unique");
        }
        objectUuids.add(object.uuid);
    }
    for (const entityUuid of Object.keys(world.equipment_by_entity)) {
        if (!entityUuids.has(entityUuid)) {
            throw new ContractValidationError("$render.objective.equipment_by_entity", "equipment row references an absent entity");
        }
    }
}
function assertVisibilityLightCoverage(row, path) {
    ...
    if (!sameSet(visibleKeys, lightKeys)) {
        throw new ContractValidationError(`${path}.effective_light_levels`, "effective-light keys must exactly cover visible cells");
    }
}
function canonicalEdgeOwner(position, direction) {
    switch (direction) {
        case "north":
...
function mergeStructuralEdge(edgeKey, owner, candidates) {
    ...
    for (const candidate of current) {
        ...
        if (appearance === null)
            continue;
        appearances.set(`${appearance.kind}:${String(appearance.is_open)}`, appearance);
    }
    if (appearances.size !== 1) {
        throw new ContractValidationError(`$render.structural_edges.${position[0]},${position[1]}:${direction}`, "equally current structural-edge appearances conflict");
    }
    const appearance = appearances.values().next().value;
    if (appearance === undefined) {
        throw new ContractValidationError("$render.structural_edges", "specific structural edge has no appearance");
    }
    return {
        edge_key: edgeKey,
        position,
...
function mergedCandidateChannels(candidates) {
...
function assertStructuralEdgeAppearance(appearance, path) {
    if (appearance.kind === "door") {
        if (appearance.is_open === null) {
            throw new ContractValidationError(`${path}.is_open`, "door structural edge requires explicit open state");
        }
        return;
    }
    if (appearance.is_open !== null) {
        throw new ContractValidationError(`${path}.is_open`, "wall structural edge cannot carry open state");
    }
}
function tileBlocks(tile, channel, direction) {
    switch (channel) {
        case "movement":
...
function readEnumArray(value, allowed, path) {
    ...
    return sortedUnique(value, path);
}
...
function readSafePresentationRef(value) {
    if (value === undefined || value === null)
        return null;
    return decodeModel("SafeContentPresentationRef", value);
}
function sortDirections(values) {
    return [...values].sort((left, right) => (DIRECTIONS.indexOf(left) - DIRECTIONS.indexOf(right)));
}
function sortChannels(values) {
    return [...values].sort((left, right) => (CHANNELS.indexOf(left) - CHANNELS.indexOf(right)));
}
...
function compareTile(left, right) {
    return left.y - right.y || left.x - right.x;
}
function compareStructuralEdge(left, right) {
...
function compareSenseMode(left, right) {
    return compareString(left.sense_type, right.sense_type)
        || left.range_feet - right.range_feet;
}
function compareVisualLayer(left, right) {
    return compareVisualSlot(left.slot, right.slot);
}
function compareVisualSlot(left, right) {
    const leftIndex = VISUAL_SLOTS.indexOf(left);
    const rightIndex = VISUAL_SLOTS.indexOf(right);
...
//# sourceMappingURL=renderProjection.js.map