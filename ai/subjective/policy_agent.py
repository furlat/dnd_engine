"""Transport-independent subjective policy controller.

The controller consumes only the canonical subjective-runtime surface. HTTP,
SSE, process readiness, and server embedding remain composition concerns.
"""
...
class SubjectivePolicyRuntime(Protocol):
    """Canonical runtime surface consumed by the shared policy loop."""

    session_id: str
    store: SubjectiveStore
    last_command_timing: Optional[dict[str, Any]]

    def bootstrap(self) -> None:
        """Initialize the subjective store."""
        ...

    def wait_for_epoch(self) -> DecisionEpoch:
        """Wait for and return the next controlled decision epoch."""
        ...
    ...
    def execute(
        ...
        **options: Any,
        ...
        """Execute one canonical affordance row."""
        ...

    def end_turn(self, *, command_id: Optional[str] = None) -> CommandResult:
        """End the current controlled turn."""
        ...

    def resync(self) -> None:
        """Recover the local subjective store from its canonical snapshot."""
        ...
    ...
    def emit_event(
        ...
        *,
        ...
        """Emit one runtime telemetry event."""
        ...

    def emit_policy_decision(self, event: PolicyDecisionTelemetry) -> None:
        """Emit one canonical policy-decision event."""
        ...


class SubjectivePolicyAgent:
    """Execute the shared policy against an injected subjective runtime."""

    def __init__(
        self,
        runtime: SubjectivePolicyRuntime,
        *,
        policy_id: str = "subjective.default",
        controller_mode: str = "subjective_ai",
        trace_event_prefix: str = "subjective_ai",
    ) -> None:
        """Create a policy agent without taking ownership of its runtime."""
        self.runtime = runtime
        self.session_id = runtime.session_id
        self.trace_event_prefix = trace_event_prefix
    ...
    def close(self, timeout_seconds: float = 4.0) -> None:
        """Flush and close policy-owned telemetry resources."""
        self.policy_telemetry.close(timeout_seconds)
    ...
    def execute_command(
        ...
        *,
        ...
        """Execute one behavior-tree command through the injected runtime."""
        ...
        if command.command_type == AgentCommandType.END_TURN:
            ...
            self.trace(
                ...
                runtime_timing=self.runtime.last_command_timing,
            )
        ...
        if command.command_type == AgentCommandType.EXECUTE:
            logger.warning(
                "execute command has no row_id: %s",
                command.model_dump(mode="json"),
            )
        return None
    ...
    def play_current_turn(
        ...
        """Play commands while the injected runtime owns the active turn."""
        ...
        for command_index in range(max_commands):
            ...
            if command is None:
                raise RuntimeError(
                    "PolicyHost selected a row absent from its canonical fact index"
                )
            policy_ms = _elapsed_ms(policy_started)
            trace_context = _shared_trace_context(
                materialized,
                facts,
                actor_uuid,
                epoch,
            ...
            if result is not None:
                host_result = self.policy_host.record_result(
                    CommandResult.model_validate(result)
                )
                self.trace(
                    "policy_host_result",
                    actor_uuid=actor_uuid,
            ...
            failures = 0
            if (
                _control_command_was_resisted(command, result)
            ...
            materialized = self.runtime.store.world or materialized
            current_epoch = (
                self.runtime.store.world.current_epoch
                if self.runtime.store.world is not None
                else None
            )
    ...
    def trace(self, event: str, **fields: Any) -> None:
        """Emit one structured AI trace through the injected runtime."""
        payload = {
            "event": event,
            "session_id": self.session_id,
            **fields,
        ...
        self.runtime.emit_event(
            f"{self.trace_event_prefix}.{event}",
            event.replace("_", " "),
    ...
    def run_once(self) -> None:
        """Run one command-selection pass."""
        self.play_current_turn(max_commands=1)

    def run_forever(self) -> None:
        """Run until the runtime reports a terminal lifecycle condition."""
        while True:
...
def _shared_trace_context(
    ...
    def entity_payload(
        ...
        *,
        ...
        if include_distance:
            payload["distance_feet"] = _subjective_distance_feet(
                actor_position,
                entity.position,
            )
        return payload
...
def _control_command_was_resisted(
    command: AgentCommand,
    result: dict[str, Any],
) -> bool:
    ...
    if result.get("status") != CommandResultStatus.ACCEPTED.value:
        return False
    return (
        "control_effect" in _logical_tags_payload(command)
        and _control_effect_was_resisted(result)
    )
