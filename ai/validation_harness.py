...
class ValidationArenaInfo(BaseModel):
    """Catalog row returned by the validation arena endpoint.

    Attributes:
        arena_id: Stable arena id accepted by `/simulation/start-ai-validation`.
    ...
    arena_id: str = Field(description="Stable arena id accepted by the validation start endpoint.")
...
class ValidationStartResult(BaseModel):
    ...
    Attributes:
        ...
        status: Status reported by the validation start endpoint.
        arena_id: Arena id reported by the server.
        mode: Validation mode reported by the server.
        ai_session_id: Session id for the spawned external AI side.
        ...
        raw_response: Full server response for later artifact logging.
    """

    schedule_entry: ValidationScheduleEntry = Field(description="Schedule row used to start the run.")
    status: str = Field(description="Status reported by the validation start endpoint.")
    ...
    ai_session_id: Optional[str] = Field(default=None, description="Session id for the spawned external AI side.")
...
class ValidationHarnessClient:
    ...
    def list_arenas(self) -> list[ValidationArenaInfo]:
        """Fetch the server-side validation arena catalog.

        Returns:
            Typed validation arena catalog rows.
        """
        response = self.client.get("/simulation/ai-validation-arenas")
        _raise_for_status(response)
        payload = response.json()
        return [ValidationArenaInfo.model_validate(row) for row in payload.get("arenas", [])]
    ...
    def start_entry(self, entry: ValidationScheduleEntry) -> ValidationStartResult:
        ...
        response = self.client.post(
            ...
            params={"arena_id": entry.arena_id, "mode": entry.mode},
        )
        _raise_for_status(response)
        payload = response.json()
        return ValidationStartResult(
            ...
            arena_id=str(payload.get("arena_id", entry.arena_id)),
            mode=_validate_mode(str(payload.get("mode", entry.mode))),
            ai_session_id=_optional_str(payload.get("ai_session_id")),
            codex_session_id=_optional_str(payload.get("codex_session_id")),
            takeover_claim_id=_optional_str(payload.get("takeover_claim_id")),
            encounter_uuid=_optional_str(payload.get("encounter_uuid")),
            hero_uuid=_optional_str(payload.get("hero_uuid")),
...
def _validate_mode(value: str) -> ValidationMode:
    """Validate a mode string from server JSON."""
    if value not in {"human_hero", "codex_monsters"}:
