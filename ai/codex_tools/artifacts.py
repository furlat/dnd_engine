"""Durable raw evidence for games controlled directly by Codex.

Direct Codex evidence belongs to the Codex tooling boundary.  It is independent
of the retired Elo, gauntlet, and evaluation platform.
"""
...
from ai.codex_tools.direct_game import (
...
class DirectCodexRotationSlot(BaseModel):
    """Retained operator context assigned to one direct Codex run."""

    model_config = ConfigDict(extra="forbid")

    sequence: int = Field(ge=0, description="Operator sequence number assigned to the run.")
    focus: str = Field(min_length=1, description="Character or controller focus recorded for this run.")
    mode: str = Field(min_length=1, description="Controller composition recorded for this run.")
    encounter_id: str = Field(
        min_length=1,
        description="Canonical encounter recipe identifier used by the run.",
    )
...
class DirectCodexRunArtifact(BaseModel):
    ...
    schema_version: Literal[2] = Field(
        default=2,
        description="Direct Codex artifact schema version.",
    )
    ...
    rotation: DirectCodexRotationSlot = Field(description="Retained operator context represented by this run.")
    ...
    @classmethod
    def migrate_legacy_artifact(cls, value: Any) -> Any:
        """Migrate the closed v1 arena identity and world-effect semantics."""
        ...
        if value.get("schema_version") == 1:
            ...
            if isinstance(rotation, dict):
                legacy_arena_id = rotation.get("arena_id")
                if (
                    isinstance(legacy_arena_id, str)
                    and "encounter_id" not in rotation
                ):
                    encounter_id = (
                        legacy_arena_id
                        if legacy_arena_id.startswith("encounter.")
                        else f"encounter.{legacy_arena_id}"
                    )
                    rotation = {
                        key: item
                        for key, item in rotation.items()
                        if key != "arena_id"
                    }
                    rotation["encounter_id"] = encounter_id
            migrated = {
                **value,
                "schema_version": 2,
                "rotation": rotation,
...
class DirectCodexStartedRun(BaseModel):
    ...
    start_result: DirectCodexGameStart = Field(description="Canonical direct Codex game startup result.")
    rotation: DirectCodexRotationSlot = Field(description="Operator context represented by the direct run.")
    ...
    def collect(
        ...
        *,
        ...
        Args:
            ...
            rotation: Retained operator context for the run.
        ...
        return DirectCodexRunArtifact(
            run_id=(
                run_id
                or _new_direct_run_id(
                    rotation.encounter_id,
                    captured_at,
                )
            ),
...
def start_direct_codex_run(
    ...
    *,
    ...
    """Start a direct Codex game and capture its initial subjective evidence.
    ...
    Args:
        ...
        rotation: Retained operator context including the exact encounter id.
    ...
    Raises:
        ValueError: If the encounter is unknown or startup cannot establish the
            exact Codex authority.
    """
    if rotation.mode != "codex_roster":
        raise ValueError(
            "direct Codex startup requires codex_roster provenance",
        )
    with DirectCodexGameClient(base_url, client=client) as game_client:
        start_result = game_client.prepare_join_bootstrap_activate(
            rotation.encounter_id,
        )

    collector = DirectCodexArtifactCollector(base_url, client=client)
    try:
        initial_snapshot = collector.capture_initial_snapshot(
            start_result.session_id,
        )
    finally:
        collector.close()
    if set(initial_snapshot.session.controlled_entity_uuids) != set(
        start_result.controlled_entity_uuids,
    ):
        raise ValueError(
            "initial snapshot does not preserve Codex entity ownership",
        )
...
def _new_direct_run_id(encounter_id: str, captured_at: str) -> str:
    ...
    safe_encounter = "".join(
        ...
        for character in encounter_id
    )
    return (
        f"{timestamp}-{safe_encounter}-direct-codex-"
        f"{uuid4().hex[:8]}"
    )
