...
export function reduceSubjectiveWorld(world, patches) {
...
export function assertSubjectiveWorld(world, perspective, path = "$subjective.world") {
    const entities = uniqueValues(world.state.entities.map((entity) => entity.uuid), `${path}.state.entities`);
    uniqueValues(world.state.floor_objects.map((object) => object.uuid), `${path}.state.floor_objects`);
    ...
    uniqueValues(world.state.grid.tiles.map(tileKey), `${path}.state.grid.tiles`);
    const controlled = uniqueValues(perspective.controlled_entity_uuids, "$subjective.perspective.controlled_entity_uuids");
    const observers = uniqueValues(perspective.observer_entity_uuids, "$subjective.perspective.observer_entity_uuids");
    if (observers.size === 0 || !observers.has(perspective.active_observer_uuid)) {
        throw new ContractValidationError("$subjective.perspective.active_observer_uuid", "active observer must belong to a non-empty observer union");
    }
    if (perspective.kind === "controlled_knowledge_union") {
        if (controlled.size === 0 || !sameSet(controlled, observers)) {
            throw new ContractValidationError("$subjective.perspective", "controlled knowledge requires identical non-empty controlled and observer sets");
        }
    }
    else if (controlled.size !== 0) {
        throw new ContractValidationError("$subjective.perspective.controlled_entity_uuids", "spectator knowledge cannot control entities");
    }
    if (!isSubset(controlled, entities) || !isSubset(observers, entities)) {
        throw new ContractValidationError(path, "controlled and observer entities must exist in projected state");
    }
    ...
    for (const [entityUuid, loadout] of Object.entries(world.visual_loadout_by_entity)) {
        if (loadout.entity_uuid !== entityUuid) {
            throw new ContractValidationError(`${path}.visual_loadout_by_entity.${entityUuid}.entity_uuid`, "visual loadout key and entity UUID differ");
        }
    ...
    for (const [observerUuid, row] of Object.entries(world.visibility)) {
        ...
        assertExactKeys(row.effective_light_levels, visibleKeys, `${path}.visibility.${observerUuid}.effective_light_levels`);
    }
}
/** Validate nested model semantics before a frame is accepted or reduced. */
export function assertSubjectiveWorldPatch(patch, path = "$subjective.patch") {
    switch (patch.kind) {
        case "floor_object_upsert":
            assertSubjectiveFloorObject(patch.object, `${path}.object`);
            return;
        case "encounter_replace":
            if (patch.encounter !== null)
                assertSubjectiveEncounter(patch.encounter, `${path}.encounter`);
            return;
        case "observer_visibility_replace": {
            const visible = new Set(patch.visibility.visible_cells.map(([x, y]) => `${x},${y}`));
            assertExactKeys(patch.visibility.effective_light_levels, visible, `${path}.visibility.effective_light_levels`);
            return;
        }
        case "visual_loadout_replace":
...
function assertConnectorSetIdentity(connectors, path) {
    uniqueValues(connectors.map((connector) => connector.uuid), `${path}.uuid`);
    uniqueValues(connectors.map((connector) => connector.authored_id), `${path}.authored_id`);
}
/** Validate connector structure shared by objective and subjective projections. */
export function assertGridConnectorStructure(grid) {
    ...
    grid.connectors.forEach((connector, connectorIndex) => {
        ...
        if (connector.endpoints[0].position[0] === connector.endpoints[1].position[0]
            && connector.endpoints[0].position[1] === connector.endpoints[1].position[1]) {
            throw new ContractValidationError(`${path}.endpoints`, "connector endpoints must occupy two distinct positions");
        }
        connector.endpoints.forEach((endpoint, endpointIndex) => {
            ...
            if (x < grid.min_x || x > grid.max_x || y < grid.min_y || y > grid.max_y) {
                throw new ContractValidationError(`${endpointPath}.position`, "connector endpoint lies outside projected grid bounds");
            }
            const tile = tiles.get(`${x},${y}`);
            if (tile === undefined) {
                throw new ContractValidationError(`${endpointPath}.position`, "connector endpoint requires a projected support tile");
            }
            if (endpoint.elevation_feet !== tile.elevation_steps * 5) {
                throw new ContractValidationError(`${endpointPath}.elevation_feet`, "connector endpoint elevation differs from its projected support tile");
            }
        });
    });
}
function assertGridConnectorIntegrity(world, path) {
    const grid = world.state.grid;
    assertGridConnectorStructure(grid);
    const tiles = keyed(grid.tiles, tileKey);
    const currentlyVisibleCells = new Set(Object.values(world.visibility).flatMap((row) => (row.visible_cells.map(([x, y]) => `${x},${y}`))));
    grid.tiles.forEach((tile, tileIndex) => {
        ...
        if (tile.visible !== isCurrentlyVisible) {
            throw new ContractValidationError(`${path}.tiles[${tileIndex}].visible`, "tile visibility must equal the current observer-union visibility");
        }
    });
    for (const visibleCell of currentlyVisibleCells) {
        const tile = tiles.get(visibleCell);
        if (tile === undefined || !tile.visible) {
            throw new ContractValidationError(`${path}.tiles`, "every current observer-union cell requires a projected visible tile");
        }
    }
    grid.connectors.forEach((connector, connectorIndex) => {
        connector.endpoints.forEach((endpoint, endpointIndex) => {
            const endpointPath = (`${path}.connectors[${connectorIndex}].endpoints[${endpointIndex}]`);
            const [x, y] = endpoint.position;
            const tile = tiles.get(`${x},${y}`);
            if (tile === undefined)
                return;
            if (!tile.visible) {
                throw new ContractValidationError(`${endpointPath}.position`, "connector endpoint requires a currently visible support tile");
            }
            if (!currentlyVisibleCells.has(`${x},${y}`)) {
                throw new ContractValidationError(`${endpointPath}.position`, "connector endpoint requires current observer-union authorization");
            }
...
export function assertSubjectiveFloorObject(object, path = "$subjective.floor_object") {
...
export function assertSubjectiveEncounter(encounter, path = "$subjective.encounter") {
    ...
    if (encounter.current_entity_uuid === null) {
        if (encounter.current_turn_index !== null) {
            throw new ContractValidationError(`${path}.current_turn_index`, "hidden current combatant cannot expose a turn index");
        }
        return;
    }
...
function assertEntityVisualLoadout(loadout, path) {
    uniqueValues(loadout.layers.map((layer) => layer.slot), `${path}.layers`);
}
function applyDoorState(objects, patch) {
    const existing = objects.get(patch.object_uuid);
    if (existing === undefined || existing.object_kind !== "door") {
        throw new ContractValidationError("$subjective.patch.door_state.object_uuid", "door-state patch must address an existing projected door");
    }
    if (existing.position[0] !== patch.position[0] || existing.position[1] !== patch.position[1]) {
        throw new ContractValidationError("$subjective.patch.door_state.position", "door-state patch position differs from the projected door");
    }
...
function assertTileInsideGrid(tile, world, path = "$subjective.patch.tile_upsert.tile") {
    const grid = world.state.grid;
...
function uniqueValues(values, path) {
...
function assertExactKeys(record, expected, path) {
