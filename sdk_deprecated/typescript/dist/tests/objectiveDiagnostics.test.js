...
import { ContractValidationError, EVENT_CONTRACT_HASH, EVENT_CONTRACT_VERSION, ObjectiveDiagnosticsClient, ObjectiveDiagnosticsFollower, ObjectiveDiagnosticsSseDecoder, TIMELINE_CONTRACT_HASH, TIMELINE_CONTRACT_VERSION, assertSubjectiveRenderParityDiagnostics, decodeModel, } from "../index.js";
...
function objectiveEventFrame(cursor = 1) {
...
function objectiveCombatLogFrame(cursor = 1) {
    return {
        source_stream_id: "encounter",
        generation_id: "generation-a",
...
test("subjective parity validation rejects vacuous or inconsistent reports", () => {
    assert.throws(() => decodeModel("SubjectiveRenderParityDiagnosticsResponse", {
        ...subjectiveParity(),
        compared_path_count: 0,
    }), ContractValidationError);
    assert.throws(() => assertSubjectiveRenderParityDiagnostics({
        ...subjectiveParity(),
        matches: false,
    }), ContractValidationError);
});
...
test("objective SSE decoder accepts only sync, cold event, and non-null objective log frames", () => {
    ...
    assert.throws(() => new ObjectiveDiagnosticsSseDecoder().feed(encodeSse("heartbeat", {})), ContractValidationError);
    assert.throws(() => new ObjectiveDiagnosticsSseDecoder().feed(encodeSse("combat_log", {
...
test("objective event frame validation uses the exhaustive generated event contract", () => {
    assert.equal(decodeModel("GameEventFrame", objectiveEventFrame()).event.wire_type, "dnd.core.events.StepMovementEvent");
    assert.throws(() => decodeModel("GameEventFrame", {
        ...objectiveEventFrame(),
        event: {
    ...
    assert.equal(decodeModel("WireEvent", objectiveEventFrame().event).wire_type, "dnd.core.events.StepMovementEvent");
});
...
//# sourceMappingURL=objectiveDiagnostics.test.js.map