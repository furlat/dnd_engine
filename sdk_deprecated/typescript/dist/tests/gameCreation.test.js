...
import { ContractValidationError, DndEngineClient, GameDirectoryClient, } from "../index.js";
...
test("game creation client exposes only catalog, compose, preview, and exact start", async () => {
    ...
    const client = new DndEngineClient("http://engine.test", {
        fetchImplementation: async (input, init) => {
            ...
            return jsonResponse(url.pathname === "/game-creation/catalog" ? catalog : {});
        },
    });
    assert.deepEqual(await client.getGameCreationCatalog(), catalog);
    await assert.rejects(() => client.composeGameCreation(composeRequest), ContractValidationError);
    await assert.rejects(() => client.previewGameCreation(previewRequest), ContractValidationError);
    await assert.rejects(() => client.startGameCreation(startRequest), ContractValidationError);
...
test("hosted composition and preview carry exact directory ownership", async () => {
    ...
    await assert.rejects(() => client.composeGameCreation(credential, composeRequest), ContractValidationError);
    await assert.rejects(() => client.previewGameCreation(credential, previewRequest), ContractValidationError);
    assert.deepEqual(calls, [
        {
            path: "/game-creation/compose",
...
test("saved roster selection preserves the exact CAS identity in composition", async () => {
    ...
    await assert.rejects(() => client.composeGameCreation(credential, savedRosterComposeRequest), ContractValidationError);
    assert.deepEqual(captured, {
        path: "/game-creation/compose",
...
test("hosted start embeds the same normalized recipe without another request", async () => {
    ...
    await assert.rejects(() => client.createGame(request), ContractValidationError);
    assert.deepEqual(calls, [{
            path: "/games",
            body: request,
        }]);
...
//# sourceMappingURL=gameCreation.test.js.map