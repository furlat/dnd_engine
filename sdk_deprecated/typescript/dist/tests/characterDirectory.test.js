...
test("character directory client uses the one exact unversioned route family", async () => {
    ...
    const requestsThatDecodeAfterFetch = [
        ...
        () => client.getSavedEncounterRoster(credential, savedRosterId),
        () => client.replaceSavedEncounterRoster(credential, savedRosterId, replaceRosterRequest),
        () => client.deleteSavedEncounterRoster(credential, savedRosterId, 1, digest),
        () => client.createSavedEncounter(credential, saveEncounterRequest),
        () => client.listSavedEncounters(credential),
        () => client.getSavedEncounter(credential, savedEncounterId),
        () => client.replaceSavedEncounter(credential, savedEncounterId, replaceEncounterRequest),
        () => client.deleteSavedEncounter(credential, savedEncounterId, 1, digest),
        ...
        () => client.getCharacterPresentationPreferences(credential, characterId),
        () => client.updateCharacterPresentationPreferences(credential, characterId, presentationPreferencesRequest),
        () => client.getCharacterAdvancement(credential, characterId),
        () => client.grantAdminCharacterAdvancementAward(credential, characterId, adminAwardRequest),
        () => client.validateCharacterLevelUp(credential, characterId, levelUpRequest),
        () => client.levelUpCharacter(credential, characterId, levelUpRequest),
        () => client.validateCharacterRespec(credential, characterId, respecRequest),
        () => client.getCharacterRespecSeed(credential, characterId),
        () => client.respecCharacter(credential, characterId, respecRequest),
        () => client.validateCharacterLoadout(credential, characterId, loadoutRequest),
        () => client.updateCharacterLoadout(credential, characterId, loadoutRequest),
    ];
    ...
    const privateRequest = (url, method = "GET", body = null) => ({
        url,
        method,
    ...
    assert.deepEqual(requests, [
        ...
        privateRequest("/api/directory/players/me/settings", "PUT", settingsRequest),
        privateRequest("/api/character-builds/validate", "POST", creationValidationRequest),
        privateRequest("/api/character-builds/visual-preview", "POST", creationValidationRequest),
        privateRequest("/api/directory/characters", "POST", createCharacterRequest),
        privateRequest("/api/directory/characters"),
        privateRequest("/api/directory/encounter-rosters", "POST", saveRosterRequest),
        privateRequest("/api/directory/encounter-rosters"),
        privateRequest(`/api/directory/encounter-rosters/${encodedRosterId}`),
        privateRequest(`/api/directory/encounter-rosters/${encodedRosterId}`, "PUT", replaceRosterRequest),
        privateRequest(`/api/directory/encounter-rosters/${encodedRosterId}?expected_revision=1&expected_recipe_digest=${digest}`, "DELETE"),
        privateRequest("/api/directory/encounters", "POST", saveEncounterRequest),
        privateRequest("/api/directory/encounters"),
        privateRequest(`/api/directory/encounters/${encodedEncounterId}`),
        privateRequest(`/api/directory/encounters/${encodedEncounterId}`, "PUT", replaceEncounterRequest),
        privateRequest(`/api/directory/encounters/${encodedEncounterId}?expected_revision=1&expected_recipe_digest=${digest}`, "DELETE"),
        ...
        privateRequest(`/api/directory/characters/${encodedId}/presentation-preferences`),
        privateRequest(`/api/directory/characters/${encodedId}/presentation-preferences`, "PUT", presentationPreferencesRequest),
        privateRequest(`/api/directory/characters/${encodedId}/advancement`),
        privateRequest(`/api/admin/characters/${encodedId}/advancement-awards`, "POST", adminAwardRequest),
        privateRequest(`/api/directory/characters/${encodedId}/level-up/validate`, "POST", levelUpRequest),
        privateRequest(`/api/directory/characters/${encodedId}/level-up`, "POST", levelUpRequest),
        privateRequest(`/api/directory/characters/${encodedId}/respec/validate`, "POST", respecRequest),
        privateRequest(`/api/directory/characters/${encodedId}/respec/seed`),
        privateRequest(`/api/directory/characters/${encodedId}/respec`, "POST", respecRequest),
        privateRequest(`/api/directory/characters/${encodedId}/loadout/validate`, "POST", loadoutRequest),
        privateRequest(`/api/directory/characters/${encodedId}/loadout`, "PUT", loadoutRequest),
    ]);
});
test("character directory response decoding is strict", async () => {
    ...
    const client = new GameDirectoryClient("/api", { fetchImplementation });
    await assert.rejects(() => client.listCharacters(credential), (error) => {
...
test("character presentation preferences preserve flexible JSON inside a strict envelope", async () => {
    ...
    const fetchImplementation = async () => new Response(JSON.stringify(payload), {
        status: 200,
        headers: { "Content-Type": "application/json" },
    });
    const client = new GameDirectoryClient("/api", { fetchImplementation });
    assert.deepEqual(await client.getCharacterPresentationPreferences(credential, payload.character_id), payload);
});
test("creator catalog decodes only editable schema-7 creation plans", async () => {
    ...
    const premadePlans = catalog.creation_plans.filter((row) => row.plan_kind === "premade_template");
    assert.deepEqual(premadePlans.map((row) => [row.source_premade_id, row.display_name]), expectedPremades);
    assert.equal(premadePlans.find((row) => (row.source_premade_id
        === "hero.fighter_2_sorcerer_3_spellblade"))?.build.premade_id, "hero.fighter_2_sorcerer_3_spellblade");
    assert.ok(premadePlans.every((row) => row.loadout.prepared_spells.length === 0));
    ...
    assert.deepEqual(catalog.appearance_catalog.options.map((option) => option.option_id), catalog.appearance_catalog.default_selection.options.map((selection) => selection.option_id));
    assert.deepEqual(catalog.appearance_catalog.constraints, []);
    assert.equal(catalog.appearance_catalog.options.find((option) => option.option_id === "appearance.beard_tint")?.values[0]?.tint_source_option_id, "appearance.hair_tint");
});
test("character list decoding accepts only the canonical closed response", async () => {
    const fetchImplementation = async () => new Response(JSON.stringify({ characters: [] }), {
        status: 200,
        headers: { "Content-Type": "application/json" },
    });
    const client = new GameDirectoryClient("/api", { fetchImplementation });
...
//# sourceMappingURL=characterDirectory.test.js.map