...
import {
  ...
  SubjectiveBootstrapUnavailableError,
  SubjectiveReplicationClient,
  SubjectiveReplicationHttpError,
  type SubjectiveBootstrapDeferred,
} from "../index.js";
import { bootstrap } from "./fixtures.js";

const DEFERRED: SubjectiveBootstrapDeferred = {
...
test("bootstrapAttempt decodes only the exact typed source-batch deferral", async () => {
  ...
  for (const malformed of [
    ...
    await assert.rejects(
      () => invalid.bootstrapAttempt("session-a"),
      ContractValidationError,
    );
  }
});
...
test("bootstrapAttempt preserves every non-deferral 409 as an HTTP error", async () => {
  ...
  for (const payload of [
    ...
    await assert.rejects(
      () => client.bootstrapAttempt("session-a"),
      (error: unknown) => {
...
test("bootstrap retry is abortable without issuing another request", async () => {
  ...
  await assert.rejects(
    () => client.bootstrap("session-a", controller.signal),
...
test("bootstrap expiry is typed, monotonic, bounded, and retains the last deferral", async () => {
  ...
  await assert.rejects(
    () => client.bootstrap("session-a"),
    (error: unknown) => (
      error instanceof SubjectiveBootstrapUnavailableError
