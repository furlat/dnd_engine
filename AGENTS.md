...
NO GIT CHECKOUT OR ANY COMMIT OR ANY ROLLBACK ONLY USERS DOES IT
...
**The user (Tommaso) is the architect and designer of this codebase.** Codex is here to assist, not to drive. This codebase has specific design decisions - ask before assuming.
...
4. **Document and measure bugs found during implementation.** When running
   tests for a feature, if you discover a failing test or suspect a bug in
   unrelated code, **dispatch a background sub-agent** to document it in
   `KNOWN_ISSUES.md` (next to AGENTS.md) while you keep working. Every issue
   recorded as `OPEN` or `INVESTIGATING` must cite an exact deterministic
   automated reproducer. For pytest-trackable behavior, add the focused
   regression first and keep the active suite green with
   `@pytest.mark.xfail(strict=True, reason="KNOWN_ISSUES: <issue title>")`
   while the defect remains open. Do not use `skip`, weaken the assertion, or
   accept an unrelated failure. The xfail must fail for the documented reason.
   For static-analysis or toolchain defects that pytest cannot exercise, cite
   the exact checked-in automated gate and expected failing diagnostic instead.
   To close the issue, fix the implementation, remove the xfail (or failing-gate
   expectation), prove that the same reproducer passes, and only then update
   the issue status to resolved.
...
- **Every open issue has a measuring test.** An `OPEN` or `INVESTIGATING`
  `KNOWN_ISSUES.md` entry without an exact deterministic reproducer is itself
  incomplete work. Use strict xfail for an intentionally open pytest
  regression so an unexpected pass fails the suite and forces issue cleanup.
  Never close an issue from prose or manual inspection alone.
- **Tests protect active product behavior, not abandoned artifacts.** Remove
  tests whose only subject is retired documentation, parity metadata, exact
  tutorial transcripts, or incidental collection counts. Before deleting such
  a test, prove that any executable behavior it exercised is covered by a
  maintained standalone regression. Preserve unique semantic, privacy,
  lifecycle, protocol, and performance assertions; replace duplicated whole
  goldens with one authoritative owner plus focused consumer-boundary checks.
...
**Entry types**: `ATTACK`, `MOVEMENT`, `ACTION`, `SAVING_THROW`, `SKILL_CHECK`, `CONDITION_APPLIED`/`REMOVED`, `DAMAGE_TAKEN`, `HEAL`, `DEATH`, `TURN_START`/`END`, `MULTI_ENTITY_ACTION`, `SPELL_SAVE`, `SPELL_DAMAGE`, `ENTITY_SPOTTED`, `HAZARD_DETECTED`

**Typed data models** in `data` field: `AttackLogData`, `MovementLogData`, `SavingThrowLogData`, `SpellSaveLogData`, `MultiEntityLogData`, `EntitySpottedLogData`, `HazardDetectedLogData`, etc.

**Standalone combat logs** (not tied to event lifecycle): `EventQueue.push_combat_log(entry, source_entity_uuid)` — used for `ENTITY_SPOTTED` (light/perception reveals hidden enemy) and `HAZARD_DETECTED` (perception reveals hidden trap). Generated by `SpatialSensesCallback` when light changes or observer perception changes reveal entities/hazards.
...
- **Observer perception change detection** — `SpatialSensesCallback` handles CONDITION_APPLICATION/CONDITION_REMOVAL on self. Compares perception snapshots (`_last_passive_perception`, `_last_sense_modes_hash`) to detect when observer's perception capabilities change (WIS buff, Darkvision gained, etc.). On change: sense modes → full visibility recompute; passive perception only → `_refilter_all_visible_entities()`. Generates `ENTITY_SPOTTED` and `HAZARD_DETECTED` combat logs.
- **Light-driven stealth detection** — When light changes reveal a hidden entity, `_refilter_entities_at()` generates `ENTITY_SPOTTED` combat logs
...
- **`HazardFilter` enum** (`dnd/core/base_conditions.py`) — `ALL` (everyone), `ENEMIES` (only enemies of source), `NON_SOURCE` (everyone except caster). Set on `BaseCondition.hazard_filter`.
- **`condition_stealth_dc`** on `BaseCondition` — perception DC to detect the hazard. Hidden traps require observer's passive perception ≥ DC.
- **`BaseBlock.is_hazardous_for(entity_uuid)`** — checks conditions on block, evaluates `hazard_filter` + `condition_stealth_dc` vs observer's perception
- **`BaseBlock.is_enemy_of(entity_uuid)`** — faction check used by `HazardFilter.ENEMIES`. Entity overrides with faction logic.
- **Safe pathfinding** — two-pass Dijkstra: normal paths first, then `walk_in_danger=False` if any path crosses hazards. `Senses.safe_paths` stores hazard-avoiding routes.
- **`prefer_safe`** on `Move` action (default True) — uses `safe_paths` when available, falls back to `paths`
- **SpikeTrapCondition / ZoneMarkerCondition** — lightweight tile conditions for hazard detection/display. Zone spells set `marker_name`, `marker_hazard_filter`, `marker_stealth_dc` on `ZoneControlCondition`.
...
**Hazard methods**: `is_position_hazardous_for(x, y, entity_uuid)`, `is_position_hazardous(x, y)` — check tile conditions for hazards. `is_walkable_for()` and `compute_paths()` accept `walk_in_danger=False` to avoid hazardous tiles.
...
entity.senses.safe_paths    # Dict[position, List[position]] - paths avoiding hazardous tiles
...
- **Visibility + entity filtering**: On `light_changed_positions`, update `senses.visible`, entities, and objects at affected positions. On `entity_entered`/`entity_left`, O(1) dict add/remove. Generates `ENTITY_SPOTTED` combat logs when light reveals hidden entities.
- **Observer perception**: On `CONDITION_APPLICATION`/`CONDITION_REMOVAL` on self, snapshot comparison detects WIS/sense mode changes → `_refilter_all_visible_entities()` or full visibility recompute. Generates `ENTITY_SPOTTED`/`HAZARD_DETECTED` combat logs.
