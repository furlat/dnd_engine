...
def test_game_session():
    ...
    codex_session = PlayerSession(
        session_id=uuid4(),
        player_type=PlayerType.CODEX,
        name="Codex"
    )

    game.add_player(human_session)
    game.add_player(codex_session)
...
def test_session_manager():
    ...
    codex = mgr.create_session(PlayerType.CODEX, "Codex")

    assert mgr.get_session(human.session_id) == human
    assert mgr.get_session(codex.session_id) == codex
...
def test_turn_validation():
    ...
    codex_session = mgr.create_session(PlayerType.CODEX, "Codex")
    ...
    game.assign_entity(skeleton.uuid, codex_session.session_id)
    ...
    other_player = codex_session if active_player == human_session else human_session
