...
echo "3. Creating codex session..."
CODEX_SESSION=$(curl -s -X POST "$BASE_URL/session/create" \
  -H "Content-Type: application/json" \
  -d '{"player_type": "codex", "name": "CodexBot"}')
echo "Response: $CODEX_SESSION"
CODEX_SESSION_ID=$(echo $CODEX_SESSION | python3 -c "import sys, json; print(json.load(sys.stdin).get('session_id', ''))")
echo "Codex Session ID: $CODEX_SESSION_ID"
echo ""
...
# 5. Join game with codex session (auto-assigns Skeleton)
echo "5. Codex joining game..."
JOIN_CODEX=$(curl -s -X POST "$BASE_URL/game/join" \
  -H "Content-Type: application/json" \
  -d "{\"session_id\": \"$CODEX_SESSION_ID\"}")
echo "Response: $JOIN_CODEX"
echo ""
...
if [ "$CONTROLLER_TYPE" = "human" ]; then
  ...
  echo "Codex-controlled turn - using codex session"
  ACTIVE_SESSION="$CODEX_SESSION_ID"
fi
...
if [ "$CONTROLLER_TYPE" = "human" ]; then
  WRONG_SESSION="$CODEX_SESSION_ID"
else
