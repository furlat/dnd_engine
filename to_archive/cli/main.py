...
Usage:
    uv run python -m cli play          # Start interactive combat (vs AI)
    uv run python -m cli playpvp       # Start PvP combat (vs Codex)
    uv run python -m cli play --host localhost --port 8000
"""
...
def render_display(client: APIClient, state: GameState, my_entity_uuid: Optional[str] = None, pvp_mode: bool = False):
    ...
    Args:
        ...
        pvp_mode: If True, check and update Codex's connection status
    """
    ...
    if pvp_mode:
        try:
            ...
            codex_connected = pvp_status.get("codex_connected", False)
            display.set_session_info(codex_connected=codex_connected)
        except Exception as e:
...
def wait_for_opponent_turn(client: APIClient, state: GameState, hero_uuid: str) -> bool:
    """
    Wait while opponent (Codex) takes their turn in PvP mode.
    ...
    while True:
        try:
            ...
            if response.status_code == 200:
                ...
                if codex_connected and not last_connected:
                    display.set_session_info(codex_connected=True)
                    last_connected = True
                elif not codex_connected and not shown_waiting:
                    display.set_session_info(codex_connected=False)
                    shown_waiting = True
...
def play(
    ...
    try:
        ...
        display.show_info("Make sure the server is running: uv run python -m server.event_server")
        raise typer.Exit(1)
...
def playpvp(
    ...
    """
    Start a PvP combat session (User vs Codex).

    You control the Hero, Codex controls the Skeletons via agent CLI.
    ...
    try:
        ...
        display.console.print(f"[green]Connected to server![/green]")
        display.console.print("[bold yellow]PvP MODE: You (Hero) vs Codex (Skeleton)[/bold yellow]")
        display.console.print("[dim]Press Enter to begin combat...[/dim]")
        input()
        ...
        display.console.print(f"[red]Codex controls: Skeleton[/red]")
        display.console.print(f"[cyan]First turn: {entity_name}[/cyan]")
        display.console.print("[dim]Waiting for Codex to connect...[/dim]")
...
def spectate(
    ...
    host: str = typer.Option("localhost", "--host", "-h", help="Server hostname"),
    port: int = typer.Option(8000, "--port", "-p", help="Server port"),
    dry_run: bool = typer.Option(False, "--dry-run", help="Skip Codex subprocesses (test the loop)"),
    model: str = typer.Option("", "--model", "-m", help="Codex model for subprocesses"),
    ...
    """
    Watch Codex vs Codex play D&D. Spawns orchestrator, shows Rich display.

    Usage: uv run python -m cli spectate [fighter|barbarian|sorcerer]
           uv run python -m cli spectate sorcerer --model gpt-5
           uv run python -m cli spectate sorcerer --fov self  (see from active entity's POV)
    """
    import os
    import signal
    import subprocess
    ...
    try:
        ...
        display.console.print("[dim]  uv run python -m server.event_server[/dim]")
        raise typer.Exit(1)
    ...
    orch_cmd = ["uv", "run", "python", "-m", "cli.orchestrator",
