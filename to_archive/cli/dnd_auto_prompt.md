...
The runtime turn prompt includes the exact command prefix for the current
entity. It includes both your session token and a turn-specific
`--expect-entity` guard. Use that exact prefix for every game command.
...
Example shape: `uv run python -m cli.agent --token {TOKEN} --expect-entity <uuid> move 5 3`
