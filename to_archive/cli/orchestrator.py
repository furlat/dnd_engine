"""PvP Orchestrator: Codex vs Codex with spectator mode.

Run from a terminal:
    uv run python -m cli.orchestrator

Flags:
    --dry-run    Skip Codex subprocesses, just end turns (for testing the loop)
    --best-of N  Run a best-of-N series (default: 1 = single match)

Launches two Codex subprocesses (hero + monsters) that take turns
...
def _cleanup_subprocesses() -> None:
    """Kill all tracked Codex subprocesses."""
...
for _i, _arg in enumerate(sys.argv):
    if _arg in {"--help", "-h"}:
        print(
            "Usage: uv run python -m cli.orchestrator "
            "[--dry-run] [--character-class fighter|barbarian|sorcerer] "
            "[--model MODEL] [--run-dir PATH] [--best-of N]"
        )
        sys.exit(0)
    if _arg == "--character-class" and _i + 1 < len(sys.argv):
        CHARACTER_CLASS = sys.argv[_i + 1]
    if _arg == "--model" and _i + 1 < len(sys.argv):
        CODEX_MODEL = sys.argv[_i + 1]
...
def start_server() -> subprocess.Popen:
    ...
    proc = subprocess.Popen(
        ["uv", "run", "python", "-m", "uvicorn", "server.event_server:app",
...
def _codex_env() -> dict:
    """Return the environment used for Codex subprocesses."""
    return os.environ.copy()


def _split_shell_command(command_text: str) -> List[str]:
    """Split a shell command, unwrapping the common bash -lc wrapper."""
    try:
        argv = shlex.split(command_text)
    except ValueError:
        return command_text.split()
    if len(argv) >= 3 and Path(argv[0]).name in {"bash", "sh"} and argv[1] == "-lc":
        try:
...
def _extract_agent_action(command_text: str, token: str) -> Optional[str]:
    """Return the cli.agent action portion from a Codex command execution."""
    argv = _split_shell_command(command_text)
    if not argv:
        return None

    token_index: Optional[int] = None
    for i, part in enumerate(argv):
        if part == "--token" and i + 1 < len(argv) and (not token or argv[i + 1] == token):
            token_index = i
            break
        if part.startswith("--token=") and (not token or part == f"--token={token}"):
            token_index = i
            break
    ...
    if "--token" in argv:
        action_parts = argv[token_index + 2 :]
    else:
        action_parts = argv[token_index + 1 :]

    if len(action_parts) >= 2 and action_parts[0] == "--expect-entity":
        action_parts = action_parts[2:]
...
def append_trajectory_turn(
    ...
    Args:
        raw_output: Raw stream-json output from Codex subprocess
        prompt: The exact turn prompt sent to Codex
    ...
    for raw_line in raw_output.split("\n"):
        ...
        if event_type == "item.completed":
            ...
            if item_type == "agent_message":
                ...
                action = _extract_agent_action(command_text, token) if token else None
                lines.append(f"**Step {step_num}** — Bash")
                if action:
                    lines.append(f"Input: `{action}`")
                else:
                    if token:
                        command_text = command_text.replace(f"--token {token}", "--token ***")
                    lines.append(f"Input: `{command_text}`")
                lines.append("Response:")
                lines.append("~~~~")
                lines.append(str(item.get("aggregated_output", "")))
                lines.append("~~~~\n")
            continue
...
def display_stream_event(event: dict, slot_id: str) -> None:
    ...
    if event_type in {"item.started", "item.completed"}:
        ...
        if item_type == "agent_message" and event_type == "item.completed":
            text = item.get("text", "").strip()
            if text:
                log(f"  [{slot_id}] {text.splitlines()[0]}")
        elif item_type == "command_execution":
            command_text = item.get("command", "")
            if event_type == "item.started":
                action = _extract_agent_action(command_text, "")
                if action:
                    log(f"  [{slot_id}] > {action}")
                elif "cli.agent" in command_text:
                    parts = command_text.split("cli.agent")[-1].strip()
                    log(f"  [{slot_id}] > {parts}")
            elif event_type == "item.completed":
                content = item.get("aggregated_output", "")
                if content and isinstance(content, str):
                    log(f"  [{slot_id}]   {content.splitlines()[0]}")
        return
...
def _contains_turn_ended(value: object) -> bool:
    """Return whether a decoded JSON value contains the turn-ended marker."""
    if isinstance(value, str):
        return "OK: Turn ended" in value
    if isinstance(value, dict):
        return any(_contains_turn_ended(child) for child in value.values())
    if isinstance(value, list):
        return any(_contains_turn_ended(child) for child in value)
    return False
...
@dataclass
class TurnResult:
    """Result from a Codex subprocess turn, supporting background drain."""
    ...
    _first_output_time: float = 0.0
    _first_action_time: float = 0.0
    _prompt_build_s: float = 0.0
    ...
    def build_metrics(self, slot_id: str, entity_name: str, round_num: int, match_num: int = 1) -> TurnMetrics:
        ...
        usage = rd.get("usage", {})
        cache_read = usage.get("cache_read_input_tokens", usage.get("cached_input_tokens", 0))
...
def _classify_stream_event(result: TurnResult, event: dict, token: str) -> None:
    ...
    if event_type == "thread.started":
        result.session_id = event.get("thread_id", result.session_id)
        return
    ...
    if event_type == "item.started":
        ...
        if isinstance(item, dict) and item.get("type") == "command_execution":
            command_text = item.get("command", "")
            if _extract_agent_action(command_text, token) and result._first_action_time == 0.0:
    ...
    if event_type == "item.completed":
        ...
        if item_type == "agent_message":
            ...
            action_name = _extract_agent_action(item.get("command", ""), token)
            if action_name:
...
def build_codex_exec_invocation(
    ...
    expected_entity_uuid: Optional[str] = None,
) -> Tuple[List[str], str]:
    """Build the Codex CLI command and stdin prompt for one turn.

    Args:
        slot: Player slot being prompted.
        prompt: Turn prompt or reflection prompt.
        max_turns: Retained for call-site compatibility; Codex CLI does not
            expose the same flag.
        allow_write: Whether the prompt should permit file-writing work.
        expected_entity_uuid: Active entity UUID captured at prompt build time.

    Returns:
        Tuple of subprocess command argv and prompt text to send on stdin.
    """
    _ = max_turns
    command_prefix = f"uv run python -m cli.agent --token {slot.token}"
    if expected_entity_uuid:
        command_prefix += f" --expect-entity {expected_entity_uuid}"

    system_prompt = slot.prompt_file.read_text() if slot.prompt_file and slot.prompt_file.exists() else ""
    write_policy = (
        "You may use file-writing tools only for the requested reflection/notebook work."
        if allow_write
        else "Do not write files during this game turn."
    )
    runtime_guard = (
        "## Runtime Command Guard\n"
        f"Every game command this turn must use this exact prefix:\n"
        "```\n"
        f"{command_prefix} <command>\n"
        "```\n"
        f"{write_policy}\n"
        "Run `end` as the final game command for the turn.\n"
    )
    stdin_prompt = "\n\n".join(part for part in (system_prompt, runtime_guard, prompt) if part)
    ...
    if CODEX_MODEL:
        common_flags += ["--model", CODEX_MODEL]

    if slot.codex_session_id:
        cmd = ["codex", "exec", "resume", *common_flags, slot.codex_session_id, "-"]
    else:
        cmd = ["codex", "exec", *common_flags, "--cd", str(PROJECT_DIR), "-"]
...
def run_codex_turn(
    ...
    """Run a Codex turn as a subprocess.

    Uses Codex JSONL output for real-time spectator view. When ``OK: Turn
    ended`` is detected in the stream, returns early while a background thread
    drains the remaining output.
    """
    cmd, stdin_prompt = build_codex_exec_invocation(
        slot,
        prompt,
        max_turns=max_turns,
        allow_write=allow_write,
        expected_entity_uuid=expected_entity_uuid,
    )
    log(f"  Invoking Codex ({slot.slot_id})...")
    ...
    try:
        ...
        log(f"  TIMEOUT: Codex ({slot.slot_id}) exceeded 3 minutes. Killing.")
        process.kill()
        process.wait()
        result.returncode = process.returncode
...
def run_post_game_reflection(
    ...
    """After encounter ends, ask Codex to write reflections.
    ...
    result = run_codex_turn(slot, prompt, max_turns=5)
...
def _flush_turn_log(pending: _PendingLog, slot_clients: Dict[str, APIClient]) -> None:
    ...
    if pending.result.session_id:
        pending.slot.codex_session_id = pending.result.session_id
    ...
    while True:
        ...
        if DRY_RUN:
            ...
            result = run_codex_turn(
                ...
                prompt,
                allow_write=False,
                expected_entity_uuid=active_uuid,
            )
...
def run_single_match(
    ...
    sessions, and runs the turn loop. Codex sessions and notebooks persist.
...
def main() -> None:
    if DRY_RUN:
        log("=== DRY RUN MODE (no Codex subprocesses) ===")
    is_series = BEST_OF > 1
    if is_series:
