...
def presentation_inventory(data: AnimationData, *, spell_ids: Iterable[str] = (),
    ...
    for identity in sorted((set(spell_ids) | data.drafts.keys()) - data.action_deliveries.keys()):
        ...
        if draft is None:
            ...
            continue
        body_only = draft.projectile is None and draft.area is None and not draft.media
    ...
    for identity in sorted(data.body_action_recipes.keys() | data.body_action_bindings.keys() | data.action_deliveries.keys()):
        if identity in data.action_deliveries:
            selected = data.action_deliveries[identity]
            rows.append(_row("action", identity, "cast", "timeline", "binding_selected", binding=selected))
            continue
    ...
    for identity, recipe in sorted(data.condition_recipes.items()):
        limitations = (*persistent_limitations(recipe, data.condition_media),
                        *transition_limitations(identity, recipe.application, data.condition_media),
                        *transition_limitations(identity, recipe.removal, data.condition_media))
    ...
    for state, pose in sorted(data.life_state_context.bodyPoses.items()):
        rows.append(_row("lifecycle", state, "lifecycle", "state-selected body pose",
                         "binding_selected", binding=pose.bodyPose))
...
def lineage_coverage(lineage: PlayerLineage, *, group: BoundChoreography | None = None,
    ...
    def visit_group(value: BoundChoreography) -> None:
        ...
        for transition in value.world_transitions:
            if transition.destruction is not None:
                world.add(transition.destruction.item_id)
            elif transition.field in ("trap_state", "pressed", "activation", "creation"):
    ...
    for event in lineage.events:
        ...
        state_only = (fact.kind in STATE_PRESENTATION_KINDS
                      or isinstance(fact, ConditionChangeFact)
                      and fact.event_type is EventType.CONDITION_STATE_CHANGED)
