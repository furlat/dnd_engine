"""Decision-epoch construction for subjective controller runtimes."""
...
from server.session import GameSession, PlayerSession
...
from server.agent_runtime.action_semantics import (
...
def build_decision_epoch(
    ...
    if (
        ...
        or encounter.turn_state != TurnState.IN_PROGRESS
    ):
        return None

    started = time.perf_counter()
    active_uuid = game.active_entity_uuid
    if active_uuid is None or active_uuid not in session.controlled_entities:
        _record_timing(record_timing, "resolve_active_actor_ms", started)
        return None
    _record_timing(record_timing, "resolve_active_actor_ms", started)
    ...
    started = time.perf_counter()
    epoch_id = (
        f"{session.session_id}:actor={actor.uuid}:round={encounter.round_number}:"
        f"turn={encounter.current_turn_index}:obs={observation_cursor}:reason={reason.value}"
    )
