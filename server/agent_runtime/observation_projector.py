...
def _entity_fact(
    ...
    if include_live_details:
        hp, normal_hp, temporary_hp, max_hp, life_state, is_dead, healing_blocked = _entity_health_details(entity)
    else:
        hp = normal_hp = temporary_hp = max_hp = life_state = is_dead = healing_blocked = None
...
def _entity_hit_point_patch_if_known(
    ...
    if entity_uuid not in session.controlled_entities and not _entity_visible_to_session(entity_uuid, observers):
        return None
    hp, normal_hp, temporary_hp, _max_hp_value, life_state, is_dead, healing_blocked = _entity_health_details(entity)
...
def _entity_health_details(entity: Entity) -> Tuple[int, int, int, int, LifeState, bool, bool]:
    """Return visible hit-point, authoritative lifecycle, and healing facts."""
