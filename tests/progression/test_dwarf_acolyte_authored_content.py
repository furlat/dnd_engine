...
def test_acolyte_starting_holdings_are_exact_ordered_srd_possessions() -> None:
    ...
    assert tuple(
        ...
        ("gear.holy_symbol", 1, None),
        ("gear.prayer_book", 1, None),
        ("gear.incense", 5, None),
        ("gear.vestments", 1, None),
        ("gear.common_clothes", 1, None),
    )
...
def test_new_srd_items_materialize_through_ordinary_registered_factories(
    ...
    owner_uuid = uuid4()
    light_hammer = build_authored_item("weapon.light_hammer", owner_uuid)
    assert light_hammer.item_id == "weapon.light_hammer"
    ...
    holy_symbol = build_authored_item("gear.holy_symbol", owner_uuid)
    prayer_book = build_authored_item("gear.prayer_book", owner_uuid)
    incense = build_authored_item("gear.incense", owner_uuid)
    assert holy_symbol.item_id == "gear.holy_symbol"
    assert prayer_book.item_id == "gear.prayer_book"
    assert incense.item_id == "gear.incense"
...
def test_dwarf_weapon_training_materializes_and_reverses_all_exact_refs(
    ...
    assert all(
        entity.creature_proficiencies.is_weapon_proficient((), item_id)
    ...
    assert all(
        not entity.creature_proficiencies.is_weapon_proficient((), item_id)
        for item_id in expected_ids
    )
