"""Engine book parity tests for spellcasting core."""
...
from dnd.actions_functional import apply_action_overrides, clear_action_overrides, execute_by_index, get_available_actions, register_spell
...
def test_eb_14_011_spell_registration_uses_templates_and_setup_order_matters() -> None:
    """EB-14-011: spell registration is exact-name template registration."""
    reset_spell_state()
    caster = create_spellcaster(spell_slots={1: 1})
    ...
    try:
        register_spells_by_name(caster, ["Not A Spell"])
        assert False, "Unknown spell name registered"
    except ValueError as exc:
        assert "Unknown spell" in str(exc)
