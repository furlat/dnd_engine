...
def test_native_routes_keep_actual_costs_conditions_terrain_and_replayable_steps(
    ...
    for lineage, public_root in zip(roots, received, strict=True):
        ...
        motion = bind_motion(player_latest, public_root, data)
        ...
        if isinstance(lineage.root, (MovementEvent, JumpEvent)):
            ...
            assert visible_contact(end).grid == lineage.root.end_position and visible_contact(end).body_lift_px == 0
            assert visible_contact(end).elevation_steps == after.tiles[lineage.root.end_position].elevation_steps
            assert visible_contact(sample_motion(motion, data, 0)).grid == lineage.root.start_position
    ...
    if behavior == "action.jump":
        ...
        assert visible_contact(middle).grid == tuple((start + end) / 2 for start, end in zip(route[0], route[-1]))
        assert middle.lift_px == arc > 0
        assert visible_contact(middle).elevation_steps == pytest.approx(
        ...
        if battlefield == "battlefield.standard_hazards_open":
            ...
            assert visible_contact(middle).grid == (2, 1) and middle.lift_px > 0
        if route == ((33, 28), (35, 26)):
            ...
            assert visible_contact(middle).grid == (34, 27) and middle.lift_px > 0
...
def test_multicell_jump_resolves_its_actual_reaction_before_takeoff_or_stops_grounded(
    ...
    assert visible_contact(held).grid == lineage.root.start_position
    ...
    assert (visible_contact(during).grid, during.lift_px) == (visible_contact(held).grid, held.lift_px)
    ...
    if survives:
        ...
        assert [visible_body(sample).frame for sample in samples] == list(range(count))
        assert all(visible_body(sample).clip == motion.clip and sample.reaction is None for sample in samples)
    else:
        assert motion.complete_ms == reaction.end_ms
        assert visible_contact(final).life_state is LifeState.DEAD
        assert (visible_contact(final).grid, visible_contact(final).body_lift_px) == (visible_contact(held).grid, held.lift_px)
        ...
        if stopped_step.path_index > 1:
            assert stopped_step.from_position != visible_contact(final).grid
...
def test_multiple_native_opportunity_attacks_join_on_the_ground_before_one_jump(data: AnimationData) -> None:
    ...
    for reaction in motion.reactions:
        ...
        for time in (reaction.start_ms, (reaction.start_ms + reaction.end_ms) / 2, reaction.end_ms - .001):
            ...
            assert visible_contact(sample).grid == lineage.root.start_position
            assert sample.lift_px == visible_contact(sample).body_lift_px == 0
            assert visible_body(sample).clip != data.movement_context.jumpClip
        elapsed = reaction.end_ms
    takeoff = sample_motion(motion, data, elapsed)
    assert takeoff.reaction is None and visible_body(takeoff).frame == 0
    assert visible_contact(takeoff).grid == lineage.root.start_position and takeoff.lift_px == 0
    landed = sample_motion(motion, data, motion.complete_ms)
    assert landed.complete and visible_contact(landed).grid == lineage.root.end_position and landed.lift_px == 0
    assert visible_contact(landed).hp == reduce_lineage(before, lineage).actors[lineage.root.source_entity_uuid].normal_hp
...
def test_jump_draws_one_complete_clip_over_its_airtime_and_lands_without_a_camera_snap(
    ...
    after = reduce_player(before, lineage)
    ...
    try:
        ...
        for quadrant in range(4):
            ...
            for index in range(count):
                ...
                assert visible_body(sampled).clip == motion.clip and visible_body(sampled).frame == index
            ...
            assert visible_body(sample_motion(motion, data, 0)).frame == 0
            assert visible_body(sample_motion(motion, data, motion.complete_ms - .001)).frame == count - 1
            for time in (0, motion.complete_ms / 2, motion.complete_ms):
                ...
                assert actor.contact == visible_contact(sampled)
                assert command[4][1] == shadow[4][1] == visible_contact(sampled).grid
                if time == motion.complete_ms / 2:
                    ...
                    assert body_depth > shadow_depth and visible_contact(sampled).body_lift_px > 0
