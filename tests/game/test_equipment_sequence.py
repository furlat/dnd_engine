...
def test_same_turn_replacement_keeps_real_roots_and_native_item_ac(
    history: tuple[PresentationTarget, tuple[CompletedLineage, ...]],
) -> None:
    before, roots = history
...
def test_shared_equipment_gesture_preserves_identity_until_settlement_and_attack_switches_on_entry(
    ...
    latest = before
    for root in roots:
        latest = reduce_lineage(latest, root)
    ...
    for root in roots:
        ...
        if isinstance(root.root, AttackEvent):
            ...
            if root.root.weapon_slot.value == "RANGED_MAIN":
                assert before.actors[before.observer_uuid].active_weapon_set is WeaponSet.MELEE
        ...
        if group.equipment:
            ...
            assert held.displayed.actors[before.observer_uuid].equipment == before.actors[before.observer_uuid].equipment
            assert latest.actors[before.observer_uuid].equipment != before.actors[before.observer_uuid].equipment
