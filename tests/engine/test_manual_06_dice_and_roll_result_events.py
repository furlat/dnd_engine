"""Focused checks for dice and roll result events."""
...
def test_d20_result_event_replaces_effective_roll() -> None:
    ...
    assert len(roll_event.roll_modifications) == 1
    modification = roll_event.roll_modifications[0]
    assert modification.handler_name == "Tutorial D20"
    assert modification.previous_total == 5
    assert modification.final_total == 20
    assert modification.reason == "raise low roll"


def test_damage_result_event_replaces_effective_packet_roll_only() -> None:
            ...
            DamageRollPacket(
                damage=Damage(
                    ...
                    damage_bonus=ModifiableValue.create(
                        source_entity_uuid=hero_id,
                        target_entity_uuid=goblin_id,
                        value_name="Tutorial Damage Bonus",
                        base_value=3,
                    ),
                    damage_type=DamageType.SLASHING,
                ),
                original_roll=original_damage,
                final_roll=original_damage,
            )
        ],
        phase=EventPhase.DECLARATION,
    )
    ...
    assert damage_event.damage_packets[0].original_roll == original_damage
    assert damage_event.damage_packets[0].final_roll == replacement_damage
    assert len(damage_event.roll_modifications) == 1
    modification = damage_event.roll_modifications[0]
    assert modification.handler_name == "Tutorial Damage"
    assert modification.packet_index == 0
    assert modification.previous_total == 6
    assert modification.final_total == 12
    assert modification.reason == "raise damage"
