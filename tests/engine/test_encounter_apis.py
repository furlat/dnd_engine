...
def mapeditor_object_request(
    client: TestClient,
    content_id: str,
    position: tuple[int, int],
) -> dict:
    """Select one exact placeable recipe from the public editor catalog."""
...
def test_eb_18_014_replication_identity_errors_are_explicit() -> None:
    """EB-18-014: player windows require exact stream identity."""
...
def test_eb_18_015_mapeditor_errors_report_catalog_map_and_save_context() -> None:
    ...
    with tempfile.TemporaryDirectory() as save_dir:
        ...
        try:
            ...
            unknown_recipe = ContentRecipe.create(
                ref=ContentRef(
                    pack_id="content.neurodragon",
                    definition_kind=(
                        ContentDefinitionKind.ENVIRONMENT_OBJECT
                    ),
                    content_id="environment.unknown_object",
                    content_version=1,
            ...
            unknown_object_response = client.post(
                "/mapeditor/map/objects",
                json={
                    "recipe": unknown_recipe.model_dump(mode="json"),
                    "content_set_digest": catalog["content_set_digest"],
                    "position": [0, 0],
                },
            )
            ...
            assert (
                unknown_object["requested_recipe"]
                == unknown_recipe.model_dump(mode="json")
            )
            assert (
                unknown_object["requested_content_set_digest"]
                == catalog["content_set_digest"]
            )
            assert unknown_object["requested_position"] == [0, 0]
            assert "environment.door" in {
                recipe["ref"]["content_id"]
                for recipe in unknown_object["valid_object_recipes"]
            }
            assert "consumable.healing_potion" in {
                recipe["ref"]["content_id"]
                for recipe in unknown_object["valid_loot_recipes"]
            }
