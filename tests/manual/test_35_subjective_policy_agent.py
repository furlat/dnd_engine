"""Focused checks for the retained subjective policy-agent evaluator."""
...
def test_subjective_policy_agent_uses_shared_host_for_door_routine(
...
def test_subjective_policy_agent_uses_shared_host_for_visible_combat(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_subjective_policy_agent_reranks_after_rejected_row_without_fallback(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_policy_source_snapshot_contains_only_the_shared_stack() -> None:
    ...
    assert "dnd/ai/contracts/control.py" in snapshot.source_paths
    assert "dnd/ai/contracts/semantics.py" in snapshot.source_paths
    assert "dnd/ai/runtime/action_semantics.py" in snapshot.source_paths
    assert "dnd/ai/runtime/decision_epoch.py" in snapshot.source_paths
...
def _door_world() -> SubjectiveWorldState:
    ...
    move_target = ActionTarget(
        index=2,
        position=(2, 0),
        path_cost=10,
        path=[(0, 0), (1, 0), (2, 0)],
    )
    move = ActionAffordance.model_validate({
        "row_id": "move-to-door",
...
def _combat_world(*, include_backup: bool = False) -> SubjectiveWorldState:
    ...
    primary = ActionAffordance.model_validate({
