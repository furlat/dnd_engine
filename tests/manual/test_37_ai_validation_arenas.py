"""Manual checks for AI validation arena fixtures."""
...
from dnd.items.test_items import StorageChest, TrapLever
...
def test_ai_validation_catalog_has_diverse_mechanics() -> None:
    """The arena catalog names distinct mechanical pressure surfaces."""
    specs = list_ai_validation_arena_specs()
    ids = [spec.arena_id for spec in specs]
    ...
    assert all(spec.expected_pressure for spec in specs)
    assert all(spec.map_notes for spec in specs)


def test_each_ai_validation_arena_builds_started_encounter_with_unique_positions() -> None:
    """Every validation arena returns live combatants and passive controllers."""
    for spec in list_ai_validation_arena_specs():
        arena = create_ai_validation_arena(spec.arena_id)
        actors = (arena.hero, *arena.monsters)
        positions = [actor.position for actor in actors]

        assert arena.spec == spec
        assert arena.encounter.state == EncounterState.ACTIVE
        assert arena.hero.faction == "heroes"
        assert {monster.faction for monster in arena.monsters} == {"monsters"}
...
def test_srd_undead_crypt_gives_shield_fighter_ranged_counterplay() -> None:
    ...
    arena = create_ai_validation_arena("srd_undead_crypt")
...
def test_goblin_water_skirmish_samples_goblins_caster_and_route_blockers() -> None:
    ...
    arena = create_ai_validation_arena("goblin_water_skirmish")
    grid = get_map()
...
def test_caster_crossfire_opens_door_and_mixes_frontline_ranged_and_spell_pressure() -> None:
    ...
    assert "Rage" in action_template_names(arena.hero)
...
def test_item_resource_gauntlet_gives_hero_varied_item_backed_options() -> None:
    ...
    arena = create_ai_validation_arena("item_resource_gauntlet")
...
def test_double_door_dark_hunt_has_two_closed_doors_and_no_initial_enemy_sight() -> None:
    ...
    arena = create_ai_validation_arena("double_door_dark_hunt")
...
def test_arcane_device_control_places_usable_environment_object_and_floor_pickup() -> None:
    ...
    arena = create_ai_validation_arena("arcane_device_control")
...
def test_skeleton_mark_focus_fire_has_support_marker_and_durable_target() -> None:
    ...
    arena = create_ai_validation_arena("skeleton_mark_focus_fire")
...
def test_buff_consumable_ambush_surfaces_buff_items_and_scrolls() -> None:
    ...
    arena = create_ai_validation_arena("buff_consumable_ambush")
...
def test_forced_movement_hazard_bridge_places_thunderwave_near_hazards() -> None:
    ...
    arena = create_ai_validation_arena("forced_movement_hazard_bridge")
    grid = get_map()
...
def test_support_attrition_cache_includes_wounded_ally_and_support_actions() -> None:
    ...
    arena = create_ai_validation_arena("support_attrition_cache")
...
def test_high_level_spell_resource_duel_samples_expensive_spells() -> None:
    ...
    assert {"Cone of Cold", "Cloudkill", "Hypnotic Pattern", "Slow", "Banishment"} <= action_template_names(arena.hero)
...
def test_sorcerer_barbarian_duel_uses_real_class_factories() -> None:
    ...
    assert {"Rage", "Frenzy", "Reckless Attack"} <= action_template_names(barbarian)
...
def test_class_party_mirror_scramble_uses_real_class_factories_and_features() -> None:
    ...
    arena = create_ai_validation_arena("class_party_mirror_scramble")
    ...
    assert "Rage" in action_template_names(berserker)
    assert "Reckless Attack" in action_template_names(berserker)
    ...
    assert "Second Wind" in action_template_names(archer)
    assert "Action Surge" in action_template_names(archer)
    assert equipped_item_name(archer, WeaponSlot.RANGED_MAIN) == "Longbow"
    assert {"Fireball", "Lightning Bolt", "Slow", "Haste"} <= action_template_names(sorcerer)
    assert "Quickened Spell" in action_template_names(sorcerer)
...
def test_ranged_loadout_kiting_ring_keeps_ranged_gear_and_terrain_pressure() -> None:
    ...
    arena = create_ai_validation_arena("ranged_loadout_kiting_ring")
    grid = get_map()
...
def test_concentration_control_crossroads_mixes_control_support_and_friendly_fire_pressure() -> None:
    ...
    arena = create_ai_validation_arena("concentration_control_crossroads")
    ...
    assert {"Web", "Hypnotic Pattern", "Slow", "Fireball"} <= action_template_names(controller)
    assert {"Bless", "Bane", "Aid", "Healing Word", "Shield of Faith", "Sanctuary"} <= action_template_names(support)
...
def test_darkness_reveal_labyrinth_uses_existing_vision_spell_surface() -> None:
    ...
    arena = create_ai_validation_arena("darkness_reveal_labyrinth")
...
def test_guardian_zone_shrine_samples_persistent_zone_and_healing_support() -> None:
    ...
    arena = create_ai_validation_arena("guardian_zone_shrine")
...
def test_damage_affinity_weapon_lab_surfaces_vulnerabilities_and_weapon_damage_types() -> None:
    ...
    arena = create_ai_validation_arena("damage_affinity_weapon_lab")
    ...
    club = next(action for action in actions if action.weapon_name == "Club")
    shortsword = next(action for action in actions if action.weapon_name == "Shortsword")
...
def test_resistance_weapon_counterplay_uses_non_skeleton_damage_affinities() -> None:
    ...
    arena = create_ai_validation_arena("resistance_weapon_counterplay")
...
def test_field_cache_loot_race_exposes_adjacent_chest_resource_choice() -> None:
    ...
    arena = create_ai_validation_arena("field_cache_loot_race")
...
def test_cleanse_support_triage_starts_with_impaired_allies_and_restoration() -> None:
    ...
    arena = create_ai_validation_arena("cleanse_support_triage")
...
def test_multi_target_missile_allocation_surfaces_projectiles_and_targets() -> None:
    ...
    arena = create_ai_validation_arena("multi_target_missile_allocation")
    missile_rows = [
        action
...
def test_reaction_counterspell_lab_registers_reaction_defenses() -> None:
    ...
    arena = create_ai_validation_arena("reaction_counterspell_lab")
...
def test_guardian_choke_body_block_exposes_persistent_zone_and_body_lane() -> None:
    ...
    arena = create_ai_validation_arena("guardian_choke_body_block")
...
def test_multi_object_control_room_exposes_several_nearby_object_actions() -> None:
    ...
    arena = create_ai_validation_arena("multi_object_control_room")
...
def test_unknown_ai_validation_arena_id_is_rejected() -> None:
    """Unknown catalog ids fail explicitly."""
    try:
        create_ai_validation_arena("not-a-real-arena")
    except ValueError as exc:
        assert "Unknown AI validation arena" in str(exc)
    else:
