...
class WorldObjectPlacement(BaseModel):
    ...
    def validate_shape(self) -> Self:
        ...
        if self.top_height_steps <= self.base_height_steps:
            raise ValueError("top_height_steps must be greater than base_height_steps")
        if self.kind is WorldPlacementKind.CENTER:
            if self.boundary_direction is not None:
                raise ValueError("center placement cannot define boundary_direction")
...
class BoundaryStructure(BaseModel):
    ...
    def validate_channels(self) -> Self:
        """Require a unique tuple in the canonical channel order."""
        canonical = tuple(WorldEdgeChannel)
        if any(channel not in canonical for channel in self.blocked_channels):
            raise ValueError("blocked_channels contains an unsupported channel")
        if len(set(self.blocked_channels)) != len(self.blocked_channels):
            raise ValueError("blocked_channels must not contain duplicates")
        if tuple(channel for channel in canonical if channel in self.blocked_channels) != self.blocked_channels:
