...
class SpatialRestraintSource(SpatialConditionMembershipSource):
    ...
    def create_manifestation(self, target: Entity) -> BaseCondition:
        ...
        return Restrained(
            source_entity_uuid=self.source_spatial_condition_uuid,
...
class RestrainingAreaCondition(MembershipAreaCondition):
    """Area condition that owns independent per-target restraint sources."""

    restraint_source_type: ClassVar[type[SpatialRestraintSource]]

    def restraint_check_dc(self) -> int:
        """Return the exact escape DC for this condition."""
