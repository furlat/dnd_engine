...
from dnd.types.saving_throws import SAVING_THROW_CONTEXT_KEY, SavingThrowContext
...
class Entity(BaseBlock):
    ...
    species_variant: Optional[SpeciesVariant] = Field(
        default=None,
        description="Optional concrete ancestry variant.",
    )
    background: Optional[Background] = Field(
        default=None,
        description="Optional concrete character background.",
    )
    applied_origin_state: Optional[AppliedOriginState] = Field(
        default=None,
        description="Persisted inputs required to rebuild origin mechanics.",
    )
    applied_class_levels: List[AppliedClassLevel] = Field(
        default_factory=list,
        description="Ordered, serialized class-level ledger.",
    )
    creation_committed: bool = Field(
        default=False,
        description="Whether the terminal entity-created fact has committed.",
    )
    ...
    feature_sources: Dict[str, Set[UUID]] = Field(
        default_factory=dict,
        description="Source-owned durable class, feat, and creature features.",
    )
    prepared_spell_ids: Set[str] = Field(
        default_factory=set,
        description="Direct semantic spells selected as currently prepared.",
    )
    feature_toggle_ids: Set[str] = Field(
        default_factory=set,
        description="Direct semantic opt-in features selected for this entity.",
    )
    has_ordinary_sight: bool = Field(
        default=True,
        description="Whether the entity can see visual phenomena without special senses."
    ...
    def remove_feature_source(self, feature_id: str, source_id: UUID) -> bool:
        """Remove one feature source without disturbing another provider."""
    ...
    def has_feature(self, feature_id: str) -> bool:
        """Return whether any source currently owns a semantic feature."""
        return bool(self.feature_sources.get(feature_id))

    def set_origin_identity(
        self,
        *,
        species: Species,
        species_variant: Optional[SpeciesVariant],
        background: Background,
    ) -> None:
        """Install domain-native species, variant, and background identity."""
        if self.species is not None or self.background is not None:
            raise RuntimeError("entity origin state is already installed")
        variant_species = {
            SpeciesVariant.HILL_DWARF: Species.DWARF,
            SpeciesVariant.HIGH_ELF: Species.ELF,
            SpeciesVariant.ROCK_GNOME: Species.GNOME,
            SpeciesVariant.LIGHTFOOT_HALFLING: Species.HALFLING,
        }
        if (
            species_variant is not None
            and variant_species[species_variant] is not species
        ):
            raise ValueError("species variant does not belong to species")
        self.species = species
        self.species_variant = species_variant
        self.background = background

    def clear_origin_identity(self) -> None:
        """Clear domain-native origin identity during exact rollback."""
        self.species = None
        self.species_variant = None
    ...
    def _attach_to_world(self, position: Tuple[int, int]) -> None:
        """Install committed entity occupancy for the owning Game."""
        ...
        *,
        ...
        for occupant_uuid in get_map().get_entities_at(position):
            ...
            if not occupant._blocks_walking_without_origin_traversal(
                ...
                continue
            if subjective:
                contact = self.senses.entities.get(occupant.uuid)
                if contact is None:
                    continue
            return False
        return True
    ...
    def _set_position(self, new_position: Tuple[int, int]) -> None:
        """Set objective entity position without mutating subjective projection."""
    ...
    def add_condition(self, condition: BaseCondition, context: Optional[Dict[str, Any]] = None, check_save_throw: bool = True, parent_event: Optional[Event] = None)  -> Optional[Event]:
        ...
        condition.bind_behavior_owner(origin_root_id=self.entity_kind_id)
    ...
    def _get_attack_bonuses(self, weapon_slot: WeaponSlot = WeaponSlot.MELEE_MAIN, override_ability: Optional[AbilityName] = None) -> Tuple[ModifiableValue, ModifiableValue, List[ModifiableValue], List[ModifiableValue], Range]:
        ...
        if not self.creature_proficiencies.is_weapon_proficient(
            ...
            weapon.get_semantic_key() if weapon is not None else None,
        ):
    ...
    def appears_in_entity_contacts(self) -> bool:
        """Dead entities do not remain creature perception contacts."""
        return self.health.life_state is not LifeState.DEAD
    ...
    def materialize_navigation(
        ...
        """Materialize derived navigation without changing perception projection."""
        grid = get_map()
        ...
        for position, path in paths.items():
            if (
                ...
                and all(step in self.senses.seen or step in self.senses.visible for step in path)
    ...
    @classmethod
    def materialize_all_navigation(cls, max_distance: int = 10) -> None:
        """Materialize derived navigation for every deployed entity."""
        for entity in cls.get_all_entities():
            if entity.is_deployed:
                entity.materialize_navigation(max_distance)
    ...
    def register_action(self, action: BaseAction) -> None:
        ...
        action.bind_behavior_owner(origin_root_id=self.entity_kind_id)
    ...
    def _make_action_info(
        ...
        Raises:
            ValueError: If the execution template has invalid direct identity.
        """
        template.bind_behavior_owner(origin_root_id=self.entity_kind_id)
        assert template.provided_by_id is not None
        ...
        action_info = AvailableActionInfo(
            ...
            provided_by_id=template.provided_by_id,
            origin_root_id=template.origin_root_id,
    ...
    def get_player_toggleable_handler_infos(
        ...
        Raises:
            ValueError: If a toggleable handler has invalid direct identity.
        """
        ...
        for entity_uuid, contact in self.senses.entities.items():
            ...
            if entity is None:
                contact_facts.append(
                    (str(entity_uuid), contact.position, None, None, None)
                )
                continue
            ...
            for obj_uuid, contact in self.senses.objects.items():
                ...
                if (
                    ...
                    distance = (
                        ...
                        else self.distance_to_position(contact.position)
                    )
                    valid_targets.append(AvailableTarget(
                        index=idx,
