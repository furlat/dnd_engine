"""Dependency-neutral elevation surfaces and canonical adjacent world edges."""
...
class SlopeAxis(str, Enum):
    """Cardinal axis along which a progressive surface may change height."""
...
class AdjacentEdgeKey:
    ...
    def __post_init__(self) -> None:
        ...
        if first >= second:
            raise ValueError("world-edge endpoints must use canonical lexical order")
    ...
    def between(
        ...
        """Build the canonical identity for one exact cardinal boundary."""
...
def progressive_elevation_transition(
    ...
    return (
        ...
        and first_surface_kind in (
            ElevationSurfaceKind.STAIRS,
...
def contradictory_progressive_elevation_edge(
    ...
    *,
    ...
    Ordinary cliffs remain valid. A completed map is contradictory only when
    an endpoint authors a progressive surface along an unequal-height edge but
    the closed both-endpoint rule does not admit exactly one height step.
    """
    ...
    for position, first in cells.items():
        for neighbor_position in (
            ...
            if bounds is not None and not (
                ...
                continue
            second = cells.get(neighbor_position)
            if second is None:
                second = implicit_surface
            if second is None:
                continue
