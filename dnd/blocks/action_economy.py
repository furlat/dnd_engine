...
class ActionEconomyDebitReceipt(BaseModel):
    """Exact evidence for modifiers installed by one aggregate commit."""
...
class ActionEconomy(BaseBlock):
    ...
    def consume_prevalidated(
        ...
        declaration. Declaration handlers may then install capability
        constraints before the admitted execution cost is committed.
        Rechecking against those later constraints would reject an action that
        the owner already admitted and break its event lifecycle.
    ...
    def _aggregate_channel_costs(
        ...
        return tuple(
            ActionEconomyChannelCost(
                ...
                name=" + ".join(sorted(labels.get(cost_type, set()))) or None,
            )
    ...
    def consume_aggregate_with_receipt(
        ...
        """Prove aggregate affordability before installing any debit."""
        aggregated = self._aggregate_channel_costs(costs)
        unaffordable = next(
            (
                cost
    ...
    def undo_prevalidated_debit(
        ...
        if (
            type(receipt.owner_uuid) is not UUID
        ...
        exact_modifiers: List[
            Tuple[ActionEconomyDebitHandle, NumericalModifier]
        ] = []
        for handle in receipt.handles:
            ...
            if (
                ...
                raise ValueError(
                    "debit receipt no longer names exact installed state"
                )
    ...
    def commit_fixed_costs_without_dispatch(
        ...
        *,
        ...
        """Synchronously commit disjoint channels and named resources."""
        ...
        try:
            ...
            raise FixedCostCommitError(
                "failed to install fixed channel costs"
            ) from exc
        ...
        try:
            for cost in aggregated_resources:
                ...
                if not resource.consume(cost.amount):
                    raise RuntimeError(
                        "prevalidated resource became unavailable"
                    )
        except Exception as exc:
            ...
            raise FixedCostCommitError(
                "failed to install fixed resource costs"
            ) from exc
