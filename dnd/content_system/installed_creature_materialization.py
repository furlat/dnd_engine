"""Creature materialization against an already-installed content runtime.

This lower adapter deliberately does not import content bootstrap.  Structural
character composition is part of the built-in declaration closure and must be
importable while that closure is being assembled.
"""
...
def resolve_installed_creature_recipe(
    recipe: ContentRecipe,
    *,
    runtime: ContentSystemRuntime = SERVER_CONTENT_SYSTEM_RUNTIME,
) -> ContentDeclaration:
    """Resolve one exact creature recipe from an installed frozen registry."""
    recipe.verify_integrity()
    if recipe.ref.definition_kind != ContentDefinitionKind.CREATURE:
        raise TypeError(
            "Creature materialization requires a creature recipe; got "
...
@overload
def materialize_installed_creature(
    recipe: ContentRecipe,
    *,
...
def materialize_installed_creature(
    ...
    *,
    ...
    """Construct and identity-check through an installed frozen registry."""
    resolve_installed_creature_recipe(recipe, runtime=runtime)
...
__all__ = [
    ...
    "resolve_installed_creature_recipe",
]
